...
def kanban_can_edit_kanban(user: User, board: KanbanBoard) -> bool:
    """Rename/create columns and other board-structure edits (requires kanban.edit)."""
    from app import rbac

    if rbac.user_has_permission(user, rbac.PERMISSION_ADMIN):
        return True
    if kanban_is_board_creator(user, board):
        return True
    if kanban_user_board_access(user, board) not in ("write", "admin"):
        return False
    return rbac.user_has_permission(user, rbac.PERMISSION_KANBAN_EDIT)
