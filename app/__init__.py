...
def create_app(config_class=Config):
    ...
    with app.app_context():
        ...
        try:
            ...
            app.logger.exception("module defaults failed")
        if isinstance(db_uri, str) and db_uri.startswith("sqlite:///"):
            try:
    ...
    def inject_sip():
        try:
            ...
            return {"sip_configured": sip_configured()}
        except Exception:
            return {"sip_configured": False}
    ...
    if not app.config.get("TESTING"):
        ...
        def _community_reminder_loop() -> None:
            while True:
                time.sleep(3600)
                with app.app_context():
                    try:
                    ...
                    try:
                        from app.module_email_notifications import run_scheduled_module_reminders

                        run_scheduled_module_reminders()
                    except Exception:
                        app.logger.exception("module email reminder scheduler failed")

        threading.Thread(
            target=_community_reminder_loop, daemon=True, name="community-reminders"
        ).start()

    from app.chat_signaling_socket import register_chat_signaling_handlers, socketio

    async_mode = str(app.config.get("SOCKETIO_ASYNC_MODE") or os.environ.get("SOCKETIO_ASYNC_MODE") or "threading").strip()
    socketio.init_app(app, async_mode=async_mode)
    register_chat_signaling_handlers()
