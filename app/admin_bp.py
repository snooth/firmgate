...
from app.app_version import (
    format_deploy_version,
    is_meaningless_version,
    normalize_version_label,
    record_deploy_version,
    resolve_deploy_version,
    software_display_version as _software_display_version_from_state,
)
...
SOFTWARE_DISPLAY_VERSION_DEFAULT = "v2.0"
...
def _merge_software_deploy(stored: Any) -> dict[str, Any]:
    out: dict[str, Any] = {
        ...
        "git_url": "",
        **{k: None for k in _DEPLOY_MERGE_FIELDS if k != "deploy_revision"},
        "deploy_revision": 0,
        "deployments": [],
    }
    ...
    for k in _DEPLOY_MERGE_FIELDS:
        ...
        if k == "deployments":
            ...
            continue
        if k == "deploy_revision":
            try:
                out[k] = int(v) if v is not None else 0
            except (TypeError, ValueError):
                out[k] = 0
            continue
        out[k] = v if isinstance(v, str) and v.strip() else (None if k != "deploy_revision" else 0)
    return out
...
def _current_repo_version(cwd: Path) -> str | None:
    git_ver = _git_describe_version(cwd)
    resolved = resolve_deploy_version(cwd, git_ver)
...
def _software_display_version(
    ...
    *,
    ...
    del is_git, live  # kept for call-site compatibility
    return _software_display_version_from_state(
        state,
...
@bp.route("/api/settings/sip", methods=["GET"])
@login_required
@admin_required_json
def api_sip_settings_get():
    from app.sip_client import sip_settings_for_api

    return jsonify(sip_settings_for_api())


@bp.route("/api/settings/sip", methods=["PUT"])
@login_required
@admin_required_json
def api_sip_settings_put():
    from app.sip_client import save_sip_settings, sip_settings_for_api

    payload = request.get_json(force=True, silent=True) or {}
    result = save_sip_settings(payload)
    if isinstance(result, tuple):
        body, code = result
        return jsonify(body), code
    _audit(
        ...
        True,
        {
            ...
            "domain": result.get("sip_domain") or "",
        },
    )
    return jsonify(result)


@bp.route("/api/settings/intranet-signaling", methods=["GET"])
@login_required
@admin_required_json
def api_intranet_signaling_settings_get():
    from app.intranet_signaling import signaling_settings_for_api

    return jsonify(signaling_settings_for_api())


@bp.route("/api/settings/intranet-signaling", methods=["PUT"])
@login_required
@admin_required_json
def api_intranet_signaling_settings_put():
    from app.intranet_signaling import save_intranet_signaling_settings, signaling_settings_for_api

    payload = request.get_json(force=True, silent=True) or {}
    result = save_intranet_signaling_settings(payload)
    if isinstance(result, tuple):
        body, code = result
        return jsonify(body), code
    _audit(
        "admin.intranet_signaling.save",
        "setting",
        "intranet_signaling",
        True,
        {
            "enabled": bool(result.get("enabled")),
            "max_participants": int(result.get("max_participants") or 40),
        },
    )
    return jsonify(result)
...
@bp.route("/api/settings/email-notifications", methods=["GET"])
@login_required
@admin_required_json
def api_email_notifications_get():
    from app.module_email_notifications import notification_settings_for_api

    return jsonify(notification_settings_for_api())


@bp.route("/api/settings/email-notifications", methods=["PUT"])
@login_required
@admin_required_json
def api_email_notifications_put():
    from app.module_email_notifications import save_notification_settings

    payload = request.get_json(force=True, silent=True) or {}
    if not isinstance(payload, dict):
        return jsonify({"error": "invalid payload"}), 400
    result = save_notification_settings(payload)
    _audit("admin.email_notifications.save", "setting", "module_email_notifications", True, {})
    return jsonify(result)


@bp.route("/api/settings/email-notifications/test", methods=["POST"])
@login_required
@admin_required_json
def api_email_notifications_test():
    from app.module_email_notifications import send_test_notification

    payload = request.get_json(force=True, silent=True) or {}
    mod = str(payload.get("module") or "").strip()
    to_addr = (payload.get("to") or payload.get("email") or "").strip()
    variant = str(payload.get("variant") or "").strip()
    ok, msg = send_test_notification(module=mod, to_addr=to_addr, variant=variant)
    if not ok:
        return jsonify({"ok": False, "error": msg}), 400
    _audit(
        "admin.email_notifications.test",
        "setting",
        "module_email_notifications",
        True,
        {"module": mod, "to": to_addr, "variant": variant or None},
    )
    return jsonify({"ok": True, "message": msg})


@bp.route("/api/settings/email-notifications/send-now", methods=["POST"])
@login_required
@admin_required_json
def api_email_notifications_send_now():
    from app.module_email_notifications import run_module_reminders

    payload = request.get_json(force=True, silent=True) or {}
    mod = str(payload.get("module") or "").strip()
    if not mod:
        return jsonify({"error": "module required"}), 400
    result = run_module_reminders(mod, force=True)
    _audit(
        "admin.email_notifications.send_now",
        "setting",
        "module_email_notifications",
        bool(result.get("ok", True)) and not result.get("skipped"),
        {
            "module": mod,
...
def api_kanban_notifications_send_due_reminders():
    ...
    _audit(
        "admin.kanban.notifications.send_due_reminders",
        "setting",
        "kanban_notifications",
        bool(result.get("ok")),
        {"sent": result.get("sent"), "failed": result.get("failed"), "checked": result.get("checked")},
    )
    if result.get("skipped"):
        return jsonify({"ok": False, "error": str(result.get("skipped")), **result}), 400
    return jsonify({"ok": True, **result})
    ...
    return jsonify(
        {
            ...
            "theme": theme,
            "portal_scale": portal_scale_percent(v),
        }
    )
...
def api_portal_settings_put():
    ...
    _audit(
        ...
        "portal",
        True,
        {"logo_enabled": bool(logo_enabled), "theme": theme, "portal_scale": portal_scale},
    )
    return jsonify(
        {
            "ok": True,
...
def api_software_version_get():
    ...
    file_ver = resolve_deploy_version(root, _git_describe_version(root) if is_git else None)
    # Bootstrap or repair stored state so the UI shows v2.44-style labels, not git shas.
    try:
        needs_save = False
        if is_git and live and not state.get("current_commit"):
            ...
            needs_save = True
        if file_ver and (
            not state.get("current_version")
            or is_meaningless_version(str(state.get("current_version") or ""))
        ):
            if not state.get("release_base"):
                state["release_base"] = file_ver
            rev = int(state.get("deploy_revision") or 0) or 1
            state["deploy_revision"] = rev
            state["current_version"] = format_deploy_version(file_ver, rev)
            state["display_version"] = state["current_version"]
            if not state.get("current_deployed_at"):
...
def api_software_version_upgrade_post():
    ...
    if ok:
        ...
        release_base = resolve_deploy_version(root, after_version or before_version)
        version_label = record_deploy_version(state, release_base or SOFTWARE_DISPLAY_VERSION_DEFAULT)
...
def api_software_version_rollback_post():
    ...
    if ok:
        ...
        state["current_deployed_at"] = _utc_iso()
        # Refresh versions after rollback (do not bump deploy revision).
        rolled_back = resolve_deploy_version(root) or normalize_version_label(str(state.get("previous_version") or ""))
        if rolled_back:
            state["current_version"] = rolled_back
            state["display_version"] = rolled_back
            state["release_base"] = rolled_back
        _append_deployment(
            state,
            {
                "action": "rollback",
