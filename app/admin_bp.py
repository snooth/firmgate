...
def admin_page():
    if not rbac.user_can_access_users_admin(current_user) and not rbac.user_can_approve_registrations(
        ...
        flash("You don’t have permission to access Administration.", "danger")
        return redirect(url_for("intranet.intranet_page"))
    return render_template("admin.html", **rbac.users_admin_template_context(current_user))
...
def api_office365_settings_get():
    from app.office365_service import office365_cfg, office365_settings_configured
...
def api_office365_settings_test():
    from app.office365_service import test_office365_connection
...
def api_security_clearance_settings_get():
    ...
    for it in items[:500]:
        ...
        seen.add(k)
        out.append(s)
    from app.security_clearance_store import effective_record_count
...
def api_security_encryption_get():
    from app import crypto_at_rest
...
def _force_delete_sqlite_database_files(db_path: Path) -> tuple[bool, str]:
    ...
    for candidate in _sqlite_sidecar_paths(db_path):
        ...
        if _unlink_if_exists(candidate):
            continue
        # Last resort: rename aside so a fresh file can be created at the canonical path.
        trash = candidate.with_name(f"{candidate.name}.factory-reset-trash")
        try:
            if trash.exists():
                trash.unlink()
    ...
    if db_path.exists():
        detail = "; ".join(errors) if errors else "file still present after delete attempts"
        return False, f"Could not remove database file {db_path} ({detail}). Stop other app workers and retry."
    return True, ""
