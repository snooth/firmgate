(function () {
  ...
  function setActiveTab(name, { persist = true } = {}) {
    ...
    setHidden("admin-tab-ai_settings", chosen === "ai_settings");
    setHidden("admin-tab-modules", chosen === "modules");
    setHidden("admin-tab-premium_features", chosen === "premium_features");
    ...
    if (chosen === "premium_features") loadPremiumLicense();
  ...
  function updateRegistrationSelfRegChrome(settings) {
    ...
    const hintPremium = document.getElementById("reg-self-premium-hint");
    if (!cb) return;
    const isExtranet = settings?.portal_theme === "non_core_team";
    const premiumOk = !!(premiumLicenseState?.enabled?.self_registration);
    cb.disabled = !isExtranet || !premiumOk;
    if (hint) hint.hidden = !!isExtranet;
    if (hintPremium) hintPremium.hidden = premiumOk || !isExtranet;
    if (!isExtranet) {
      cb.checked = false;
    } else if (settings) {
  ...
  const ACCESS_MATRIX_SECTIONS = [
    ...
    },
    {
      title: "CRM",
      items: [
        ["crm.read", "View CRM entries"],
        ["crm.create", "Create / edit CRM entries"],
  ...
  function wireBackupRestoreOnce() {
    ...
    if (btnFactory) {
      btnFactory.addEventListener("click", async () => {
        ...
        if (
          !window.confirm(
            "This permanently deletes ALL portal data: users, documents, CRM leads and pipeline, settings, uploads (including document files), and branding.\n\nThe portal will be restored to a fresh install with admin@example.com / admin.\n\nThis cannot be undone. Proceed?"
          )
        ) {
          return;
        }
  ...
  const ENTERPRISE_ONLY_MODULES = new Set([
    "security_clearance",
    "security_officer",
    "timesheets",
  ...
  let modulesCommunityEdition = true;
  ...
  const MODULES = [
    ...
    { key: "timesheets_collection", label: "Timesheet Collection" },
    ...
    { key: "ai_document_search", label: "AI Document Search" },
    { key: "ai_chatbot", label: "AI Chatbot" },
    { key: "ai_policy_assistant", label: "AI Docs and Policy" },
    { key: "ai_cv_builder", label: "AI CV Builder" },
    { key: "ai_tender_assistant", label: "AI Tender Assistant" },
    { key: "crm", label: "CRM" },
    { key: "about", label: "About Company" },
    { key: "resource_pool", label: "Resource Pool" },
    { key: "resource_calculator", label: "Casual Calculator" },
    { key: "game", label: "Games" },
    { key: "admin", label: "Administration" },
  ];

  let modulesState = null; // {modules:{[key]:{enabled,restricted,allowed_user_ids}}}
  ...
  /** Keep Enabled / Restricted toggle positions from the DOM in modulesState (re-render used to wipe them). */
  ...
  /** Users highlighted in the multi-select but not yet chipped are merged on Save. */
  ...
  function moduleRowHtml(m, cfg, uById, allUsers) {
      const enterpriseLicensed =
        licensedEnterpriseModules.has(m.key) ||
        !!(premiumLicenseState?.enabled && premiumLicenseState.enabled[m.key]) ||
        (!!premiumLicenseState?.enterprise_intranet_licensed &&
          (m.key === "crm" || m.key === "resource_pool" || m.key === "resource_calculator" || m.key === "security_clearance")) ||
        (m.key === "timesheets" && premiumLicenseState?.enabled?.timesheets) ||
        (m.key === "timesheets_collection" && premiumLicenseState?.enabled?.timesheets) ||
        (m.key === "ai_chatbot" && premiumLicenseState?.enabled?.ai_document_search) ||
        (m.key === "ai_policy_assistant" && premiumLicenseState?.enabled?.ai_document_search) ||
        (m.key === "ai_cv_builder" && premiumLicenseState?.enabled?.ai_document_search) ||
        (m.key === "ai_tender_assistant" && premiumLicenseState?.enabled?.ai_document_search);
      const enterpriseLocked =
        modulesCommunityEdition && ENTERPRISE_ONLY_MODULES.has(m.key) && !enterpriseLicensed;
      const enterpriseLabel =
        m.key === "ai_document_search" ||
        m.key === "ai_chatbot" ||
        m.key === "ai_policy_assistant" ||
        m.key === "ai_cv_builder" ||
        m.key === "ai_tender_assistant"
          ? "Enterprise feature"
          : "Enterprise edition";
      const row = cfg[m.key] || {};
      const enabled = enterpriseLocked ? false : row.enabled !== false;
      const restricted = enterpriseLocked ? false : !!row.restricted;
      ...
      return `<div class="nc-admin-card nc-mod-card${enabled ? "" : " nc-mod-card--off"}" style="margin-top:0.75rem;" data-mod="${escapeHtml(
        ...
        <div style="display:flex; justify-content:space-between; gap:1rem; align-items:flex-start; flex-wrap:wrap;">
          <div>
            <div style="font-weight:950; letter-spacing:-0.02em;">${escapeHtml(m.label)}${
        enterpriseLocked ? ` <span class="nc-detail-muted" style="font-weight:600;">(${escapeHtml(enterpriseLabel)})</span>` : ""
      }</div>
        ...
        enabled && !enterpriseLocked ? "" : "disabled"
      }>
                <span class="nc-switch-ui"></span>
              </label>
            </div>
        ...
        <div class="nc-detail-muted" style="margin-top:0.55rem;">
          ${
            enterpriseLocked
              ? m.key === "crm" ||
                m.key === "resource_pool" ||
                m.key === "resource_calculator" ||
                m.key === "security_clearance"
                ? `<b>${escapeHtml(enterpriseLabel)}</b> — activate a license with <code>enterprise_intranet</code>, <code>crm</code>, <code>resource_pool</code>, <code>resource_calculator</code>, and/or <code>security_clearance</code> (vendor: <code>--enterprise</code> or <code>--all</code>). AI-only keys (<code>--ai</code>) do not unlock these modules.`
                : m.key === "timesheets"
                ? `<b>${escapeHtml(enterpriseLabel)}</b> — activate a license with the <code>timesheets</code> feature (vendor: <code>--feature timesheets</code> or <code>--all</code>). Unlocks personal timesheets.`
                : m.key === "timesheets_collection"
                ? `<b>${escapeHtml(enterpriseLabel)}</b> — requires the <code>timesheets</code> license (same as Timesheets). Shows the signed-timesheet roster under Timesheets in the menu when enabled.`
                : `<b>${escapeHtml(enterpriseLabel)}</b> — activate a subscription license under <b>Enterprise Features</b> (e.g. <code>ai_document_search</code>, <code>ai_chatbot</code>, or <code>--ai</code> from your vendor).`
              : enterpriseLicensed && !enabled
              ? "<b>Licensed</b> — turn on <b>Enabled</b> to show this module in the Firmgate menu."
              : !enabled
              ? "<b>Disabled</b> — hidden from the Firmgate menu for everyone (portal admins still see <b>Administration</b> so they can change this). "
        ...
        enabled && restricted ? "" : "display:none;"
          ...
          <div style="margin-top:0.65rem;">
            ...
            <p class="nc-detail-muted" style="margin:0.25rem 0 0.35rem; font-size:0.82rem;">Select one or more users (Ctrl/Cmd+click), then <b>Save</b> below—or <b>Add to list</b> to preview chips first.</p>
  ...
  async function loadModulesSettings() {
    if (!premiumLicenseState?.valid) {
      try {
        await loadPremiumLicense();
      } catch (_e) {}
    }
    ...
    modulesState = j;
    modulesCommunityEdition = j.community_edition !== false;
    licensedEnterpriseModules = new Set(j.licensed_enterprise_modules || []);
    renderModules();
  ...
  async function saveModulesSettings({ quiet = false } = {}) {
    ...
    modulesState = jj;
    licensedEnterpriseModules = new Set(jj.licensed_enterprise_modules || []);
    renderModules();
  ...
  function updateDocumentEditorCards(provider) {
    ...
    const o365Card = document.getElementById("integrations-office365-card");
    if (ooCard) ooCard.style.opacity = p === "onlyoffice" ? "1" : "0.72";
    if (o365Card) o365Card.style.opacity = p === "office365" ? "1" : "0.72";
  }
  ...
  async function loadOffice365Settings() {
    const r = await api("/api/settings/office365", { method: "GET" });
    const j = await r.json().catch(() => ({}));
    if (!r.ok) return;
    const tenantEl = document.getElementById("o365-tenant");
    const clientIdEl = document.getElementById("o365-client-id");
    const secretEl = document.getElementById("o365-client-secret");
    const siteHostEl = document.getElementById("o365-site-host");
    const sitePathEl = document.getElementById("o365-site-path");
    const driveEl = document.getElementById("o365-drive-id");
    const stagingEl = document.getElementById("o365-staging-folder");
    const skipTlsEl = document.getElementById("o365-skip-tls");
    if (tenantEl) tenantEl.value = j.tenant_id || "";
    if (clientIdEl) clientIdEl.value = j.client_id || "";
    if (secretEl) {
      secretEl.value = "";
      secretEl.placeholder = j.client_secret_set
        ? "Leave blank to keep saved secret"
        : "Azure app client secret";
    }
    if (siteHostEl) siteHostEl.value = j.site_hostname || "";
    if (sitePathEl) sitePathEl.value = j.site_path || "";
    if (driveEl) driveEl.value = j.drive_id || "";
    if (stagingEl) stagingEl.value = j.staging_folder || "FirmgateEdits";
    if (skipTlsEl) skipTlsEl.checked = !!j.skip_tls_verify;
  }

  async function testOffice365() {
    setStatus("admin-o365-status", "Testing Microsoft 365 connection…");
    const r = await api("/api/settings/office365/test", { method: "GET" });
    const j = await r.json().catch(() => ({}));
    if (!r.ok || !j.ok) {
      const hints = (j.hints || []).length ? `\n${(j.hints || []).join(" ")}` : "";
      setStatus("admin-o365-status", (j.error || "Microsoft 365 test failed") + hints);
      return;
    }
    setStatus("admin-o365-status", j.message || "Microsoft 365 connected.");
  }
  ...
  const o365Save = document.getElementById("o365-save");
  if (o365Save) {
    o365Save.addEventListener("click", async () => {
      const tenant_id = (document.getElementById("o365-tenant").value || "").trim();
      const client_id = (document.getElementById("o365-client-id").value || "").trim();
      const client_secret = (document.getElementById("o365-client-secret").value || "").trim();
      const site_hostname = (document.getElementById("o365-site-host").value || "").trim();
      const site_path = (document.getElementById("o365-site-path").value || "").trim();
      const drive_id = (document.getElementById("o365-drive-id").value || "").trim();
      const staging_folder = (document.getElementById("o365-staging-folder").value || "").trim();
      const skip_tls_verify = !!(document.getElementById("o365-skip-tls") && document.getElementById("o365-skip-tls").checked);
      const r = await api("/api/settings/office365", {
        method: "PUT",
        body: JSON.stringify({
          tenant_id,
          client_id,
      ...
      const j = await r.json().catch(() => ({}));
      setStatus("admin-o365-status", r.ok ? "Saved." : j.error || "Save failed");
      await loadOffice365Settings();
      if (r.ok) await testOffice365();
    });
  }
  const o365Test = document.getElementById("o365-test");
  if (o365Test) o365Test.addEventListener("click", () => testOffice365());

  async function loadAdLdapSettings() {
    const r = await api("/api/settings/ad_ldap", { method: "GET" });
    const j = await r.json().catch(() => ({}));
    if (!r.ok) return;
    const uriEl = document.getElementById("ldap-uri");
    const bindDnEl = document.getElementById("ldap-bind-dn");
    const bindPwEl = document.getElementById("ldap-bind-pw");
    const baseDnEl = document.getElementById("ldap-base-dn");
    const filterEl = document.getElementById("ldap-user-filter");
    const startTlsEl = document.getElementById("ldap-start-tls");
    const skipCertEl = document.getElementById("ldap-skip-cert");
    if (uriEl) uriEl.value = j.server_uri || "";
    if (bindDnEl) bindDnEl.value = j.bind_dn || "";
    if (bindPwEl) {
      bindPwEl.value = "";
      bindPwEl.placeholder = j.bind_password_set
        ? "Leave blank to keep saved password"
        : "Service account password";
    }
    if (baseDnEl) baseDnEl.value = j.base_dn || "";
    if (filterEl) filterEl.value = j.user_search_filter || "";
    if (startTlsEl) startTlsEl.checked = !!j.start_tls;
    if (skipCertEl) skipCertEl.checked = !!j.skip_cert_verify;
  }

  async function testAdLdap() {
    setStatus("admin-ldap-status", "Testing LDAP connection…");
    const r = await api("/api/settings/ad_ldap/test", { method: "GET" });
    const j = await r.json().catch(() => ({}));
    if (!r.ok || !j.ok) {
      setStatus("admin-ldap-status", j.error || "LDAP test failed.");
      return;
    }
    setStatus("admin-ldap-status", j.message || "LDAP connection OK.");
  }

  const ldapSave = document.getElementById("ldap-save");
  if (ldapSave) {
    ldapSave.addEventListener("click", async () => {
      const server_uri = (document.getElementById("ldap-uri").value || "").trim();
      const bind_dn = (document.getElementById("ldap-bind-dn").value || "").trim();
      const bind_password = (document.getElementById("ldap-bind-pw").value || "").trim();
      const base_dn = (document.getElementById("ldap-base-dn").value || "").trim();
      const user_search_filter = (document.getElementById("ldap-user-filter").value || "").trim();
      const start_tls = !!(document.getElementById("ldap-start-tls") && document.getElementById("ldap-start-tls").checked);
      const skip_cert_verify = !!(document.getElementById("ldap-skip-cert") && document.getElementById("ldap-skip-cert").checked);
      const r = await api("/api/settings/ad_ldap", {
        method: "PUT",
        body: JSON.stringify({
          server_uri,
          bind_dn,
      ...
      const j = await r.json().catch(() => ({}));
      setStatus("admin-ldap-status", r.ok ? "Saved." : j.error || "Save failed");
      await loadAdLdapSettings();
      if (r.ok) await testAdLdap();
    });
  }
  const ldapTest = document.getElementById("ldap-test");
  if (ldapTest) ldapTest.addEventListener("click", () => testAdLdap());
  ...
  (async () => {
    ...
    const normalizeTab = (t) => (t === "ai_document_search" ? "ai_settings" : t);
  ...
  function enterpriseFeaturePill(k) {
    if (AI_ENTERPRISE_FEATURES.includes(k)) return "AI";
    if (k === "crm") return "CRM";
    if (k.startsWith("security_")) return "Security";
    if (k === "office365" || k === "ldap") return "Integration";
    return "Enterprise";
  }

  function renderPremiumFeaturesList(status) {
    const list = document.getElementById("premium-features-list");
    if (!list) return;
    const labels = status.feature_labels || {};
    const enabled = status.enabled || {};
    const keys = Object.keys(labels).sort();
    if (!keys.length) {
      list.innerHTML = '<p class="nc-intranet-muted">No enterprise features defined.</p>';
      return;
    }
    const featureRow = (k) => {
      const on = !!enabled[k];
      const label = labels[k] || k;
      return `<div class="nc-enterprise-feature">
          <span class="nc-intranet-pill nc-enterprise-feature-pill">${escapeHtml(enterpriseFeaturePill(k))}</span>
          <div class="nc-enterprise-feature-main">
            <div class="nc-enterprise-feature-title">${escapeHtml(label)}</div>
            <div class="nc-enterprise-feature-key"><code>${escapeHtml(k)}</code></div>
          </div>
          <span class="nc-enterprise-state${on ? " nc-enterprise-state--on" : ""}">${on ? "Licensed" : "Locked"}</span>
        </div>`;
    };
    const aiKeys = AI_ENTERPRISE_FEATURES.filter((k) => labels[k]);
    const otherKeys = keys.filter((k) => !AI_ENTERPRISE_FEATURES.includes(k));
    let html = "";
    if (aiKeys.length) {
      html += `<div class="nc-enterprise-features-group"><p class="nc-enterprise-features-group-label">AI</p>${aiKeys.map(featureRow).join("")}</div>`;
    }
    if (otherKeys.length) {
      html += `<div class="nc-enterprise-features-group"><p class="nc-enterprise-features-group-label">Integrations &amp; security</p>${otherKeys.map(featureRow).join("")}</div>`;
    }
    list.innerHTML = html;
  }

  function refreshPremiumLicenseUi(status) {
    renderPremiumFeaturesList(status);
    renderPremiumLicenseDetails(status);
    renderPremiumLicenseHeroBadge(status);
  }

  function renderPremiumLicenseHeroBadge(status) {
    const badge = document.getElementById("premium-license-hero-badge");
    if (!badge) return;
    const featN = (status?.features || []).length;
    badge.classList.remove("is-active", "is-expired", "is-inactive");
    if (status?.valid && !status?.expired) {
      badge.hidden = false;
      badge.classList.add("is-active");
      badge.textContent = featN ? `Active · ${featN} feature${featN === 1 ? "" : "s"}` : "Active license";
      return;
    }
    if (status?.expired) {
      badge.hidden = false;
  ...
  function renderPremiumLicenseDetails(status) {
    const box = document.getElementById("premium-license-details");
    if (!box) return;
    const subject = (status?.subject || "").trim();
    const exp = (status?.expires_at || "").trim();
    const fp = (status?.key_fingerprint || "").trim();
    const applied = (status?.applied_at || "").trim();
    const hasAnything = !!(subject || exp || fp || applied || status?.expired);
    if (!hasAnything) {
      box.hidden = true;
      box.innerHTML = "";
      return;
    ...
    const stat = (label, valueHtml) =>
      `<div><div class="nc-enterprise-license-stat-label">${escapeHtml(label)}</div><div class="nc-enterprise-license-stat-value">${valueHtml}</div></div>`;
    const rows = [];
    if (subject) rows.push(stat("Customer", escapeHtml(subject)));
    if (exp) {
      const badge = status?.expired
        ? '<span class="nc-premium-badge is-expired">Expired</span>'
        : '<span class="nc-premium-badge is-active">Active</span>';
      rows.push(stat("Expiry", `<code>${escapeHtml(exp)}</code> ${badge}`));
    } else if (status?.expired) {
      rows.push(stat("Status", '<span class="nc-premium-badge is-expired">Expired</span>'));
    }
    if (applied) rows.push(stat("Activated", `<code>${escapeHtml(applied)}</code>`));
    if (fp) rows.push(stat("Fingerprint", `<code>${escapeHtml(fp)}</code>`));
    box.hidden = false;
    box.innerHTML = `
      <div class="nc-enterprise-feature-title">Current license</div>
      <p class="nc-intranet-muted" style="margin:0.35rem 0 0;font-size:0.88rem;">Stored on this server. Expired licenses lock enterprise features without deleting your data.</p>
      <div class="nc-enterprise-license-summary-grid">${rows.join("")}</div>
    `;
  }

  function applyPremiumGates(status) {
    const enabled = status?.enabled || {};
    const aiEnterpriseLicensed =
      !!status?.ai_enterprise_licensed ||
      !!(
        enabled.ai_document_search ||
        enabled.ai_chatbot ||
        enabled.ai_policy_assistant ||
        enabled.ai_cv_builder ||
        enabled.ai_tender_assistant
      );
    document.querySelectorAll(".nc-premium-gated[data-premium-feature]").forEach((el) => {
      const feat = el.dataset.premiumFeature;
      let licensed = !!enabled[feat];
      if (feat === "ai_enterprise") {
        licensed = aiEnterpriseLicensed;
      }
      el.classList.toggle("nc-premium-locked", !licensed);
      el.querySelectorAll(".nc-premium-badge, .nc-premium-lock-msg").forEach((n) => {
        n.hidden = licensed;
      });
      el.querySelectorAll("input, select, textarea, button").forEach((ctrl) => {
        if (ctrl.closest("#admin-tab-premium_features")) return;
        if (ctrl.closest(".nc-admin-ai-settings-subnav")) return;
        if (ctrl.classList.contains("nc-admin-nav-item") && !el.classList.contains("nc-admin-nav-item")) return;
        ctrl.disabled = !licensed;
      });
    });
    const o365Opt = document.querySelector('#doc-editor-provider option[value="office365"]');
    if (o365Opt) o365Opt.disabled = !enabled.office365;
  }

  async function loadPremiumLicense() {
    const r = await api("/api/settings/premium-license", { method: "GET" });
    const j = await r.json().catch(() => ({}));
    if (!r.ok) {
      setStatus("premium-license-status", j.error || "Could not load enterprise license.");
      return;
    }
    premiumLicenseState = j;
    refreshPremiumLicenseUi(j);
    applyPremiumGates(j);
    if (document.getElementById("admin-modules-wrap")) {
      licensedEnterpriseModules = new Set(j.licensed_enterprise_modules || []);
      renderModules();
    }
    try {
      const rr = await api("/api/settings/registration", { method: "GET" });
      const rj = await rr.json().catch(() => ({}));
      if (rr.ok) updateRegistrationSelfRegChrome(rj);
    } catch (_e) {}
    const st = document.getElementById("premium-license-status");
    if (st) {
      const revokedN = Number(j.revoked_count || 0);
      const revokedHint = revokedN ? ` ${revokedN} revoked key(s) blocked.` : "";
      st.textContent = j.valid
        ? `Active license (${(j.features || []).length} enterprise feature(s)).${revokedHint}`
        : `No active enterprise license.${revokedHint}`;
      if (j.expired) {
        st.textContent = `License expired — enterprise features are locked.${revokedHint}`;
      }
      if (j.key_fingerprint && j.valid) {
        st.textContent += ` Fingerprint: ${j.key_fingerprint}.`;
      }
    }
  }

  document.getElementById("premium-license-apply")?.addEventListener("click", async () => {
    const key = (document.getElementById("premium-license-key")?.value || "").trim();
    if (!key) {
      setStatus("premium-license-status", "Paste a license key first.");
      return;
    }
    setStatus("premium-license-status", "Activating…");
    const r = await api("/api/settings/premium-license", {
      method: "PUT",
      body: JSON.stringify({ license_key: key }),
    });
    const j = await r.json().catch(() => ({}));
    if (!r.ok) {
      setStatus("premium-license-status", j.error || "Activation failed.");
      return;
    }
    async function waitForRestartThenReload() {
      const hint = (j.restart && j.restart.message) || "Restarting the app…";
      setStatus("premium-license-status", hint);
      const until = Date.now() + 45_000;
      while (Date.now() < until) {
        try {
          const rr = await fetch(u("/health"), { method: "GET", cache: "no-store" });
          if (rr.ok) break;
        } catch (_e) {}
    ...
    refreshPremiumLicenseUi(j);
    applyPremiumGates(j);
    if (j.restart && j.restart.pending) {
      waitForRestartThenReload();
      return;
    }
    setStatus("premium-license-status", "License activated.");
    loadModulesSettings();
    loadOffice365Settings();
    loadAdLdapSettings();
    loadEncryptionSettings();
    loadAiDocumentSearchSettings();
  });

  document.getElementById("premium-license-clear")?.addEventListener("click", async () => {
    if (!window.confirm("Remove the enterprise license and lock enterprise features?")) return;
    setStatus("premium-license-status", "Clearing…");
    const r = await api("/api/settings/premium-license", {
      method: "PUT",
    ...
    const j = await r.json().catch(() => ({}));
    if (!r.ok) {
      setStatus("premium-license-status", j.error || "Could not clear license.");
      return;
    }
    async function waitForRestartThenReload() {
      const hint = (j.restart && j.restart.message) || "Restarting the app…";
    ...
    setStatus("premium-license-status", "License cleared.");
    loadAiDocumentSearchSettings();
  });

  const adsSelectedFolders = new Map();
  const AI_SETTINGS_PANES = [
    "document_search",
    "chatbot",
    "policy_assistant",
    "cv_builder",
  ...
  const AI_SIMPLE_LLM = {
    policy_assistant: {
      prefix: "aipolicy",
      apiPath: "/api/settings/ai-policy-assistant",
      statusId: "admin-aipolicy-status",
      hintId: "admin-aipolicy-config-hint",
      label: "AI Docs and Policy",
      defaultTemperature: 0.35,
      defaultMaxTokens: 1800,
    },
    cv_builder: {
      prefix: "aicv",
      apiPath: "/api/settings/ai-cv-builder",
      statusId: "admin-aicv-status",
      hintId: "admin-aicv-config-hint",
      label: "AI CV Builder",
    ...
    tender_assistant: {
      prefix: "aitender",
      apiPath: "/api/settings/ai-tender-assistant",
      statusId: "admin-aitender-status",
      hintId: "admin-aitender-config-hint",
      label: "AI Tender Assistant",
      defaultTemperature: 0.2,
      defaultMaxTokens: 8192,
    },
  };

  function aiFloatInput(id, fallback) {
    const el = document.getElementById(id);
  ...
  function applyAiTempMaxFields(prefix, j, defaults) {
    const temp = document.getElementById(`${prefix}-temperature`);
    const maxTok = document.getElementById(`${prefix}-max-tokens`);
    if (temp) temp.value = j.temperature ?? defaults.temperature;
    if (maxTok) maxTok.value = j.max_tokens ?? defaults.maxTokens;
  }

  function readAiTempMaxBody(prefix, defaults) {
    return {
      temperature: aiFloatInput(`${prefix}-temperature`, defaults.temperature),
      max_tokens: adsNum(`${prefix}-max-tokens`, defaults.maxTokens),
    };
  }
  const aiSettingsApiKeys = { document_search: false, chatbot: false };

  function updateAiCopyKeyButtons() {
    const adsBtn = document.getElementById("ads-copy-api-key");
    const chatBtn = document.getElementById("aichat-copy-api-key");
    if (adsBtn) adsBtn.hidden = !aiSettingsApiKeys.chatbot;
    if (chatBtn) chatBtn.hidden = !aiSettingsApiKeys.document_search;
    for (const btn of document.querySelectorAll(".ai-copy-api-key-btn[data-copy-from='document_search']")) {
      btn.hidden = !aiSettingsApiKeys.document_search;
    }
  }

  async function copyAiApiKey(to, from = "document_search") {
    const statusByProduct = {
      document_search: "admin-ads-status",
      chatbot: "admin-aichat-status",
      policy_assistant: "admin-aipolicy-status",
      cv_builder: "admin-aicv-status",
      tender_assistant: "admin-aitender-status",
    };
    const keyIdByProduct = {
      document_search: "ads-api-key",
      chatbot: "aichat-api-key",
      policy_assistant: "aipolicy-api-key",
      cv_builder: "aicv-api-key",
      tender_assistant: "aitender-api-key",
    };
    const statusId = statusByProduct[to] || "admin-ads-status";
    setStatus(statusId, "Copying API key…");
    const r = await api("/api/settings/ai-settings/copy-api-key", {
      method: "POST",
      body: JSON.stringify({ to, from }),
    });
    const j = await r.json().catch(() => ({}));
    if (!r.ok) {
      setStatus(statusId, j.error || "Could not copy API key.");
      return;
    }
    setStatus(statusId, j.message || "API key copied.");
    const keyEl = document.getElementById(keyIdByProduct[to]);
    if (keyEl) {
      keyEl.value = "";
      keyEl.placeholder = "Saved key copied — leave blank to keep, or paste a new key";
    }
    await loadAiDocumentSearchSettings();
    await loadAiChatbotSettings();
    for (const product of Object.keys(AI_SIMPLE_LLM)) {
      await loadAiSimpleLlmSettings(product);
    }
  }

  async function loadAiSimpleLlmSettings(product) {
    const cfg = AI_SIMPLE_LLM[product];
    if (!cfg) return;
    const r = await api(cfg.apiPath, { method: "GET" });
    const j = await r.json().catch(() => ({}));
    const hint = document.getElementById(cfg.hintId);
    if (!r.ok) {
      setStatus(cfg.statusId, j.error || `Could not load ${cfg.label} settings.`);
      return;
    }
    const keyEl = document.getElementById(`${cfg.prefix}-api-key`);
    if (keyEl && j.api_key_set) {
      keyEl.placeholder = "Saved key on server — leave blank to keep, or paste to replace";
    } else if (keyEl) {
      keyEl.placeholder = "sk-… or leave blank to keep saved key";
    }
    const base = document.getElementById(`${cfg.prefix}-base-url`);
    const model = document.getElementById(`${cfg.prefix}-model`);
    const skipTls = document.getElementById(`${cfg.prefix}-skip-tls`);
    if (base) base.value = j.base_url || "";
    if (model) model.value = j.model || "gpt-4o-mini";
    if (skipTls) skipTls.checked = !!j.skip_tls_verify;
    applyAiTempMaxFields(cfg.prefix, j, {
      temperature: cfg.defaultTemperature,
      maxTokens: cfg.defaultMaxTokens,
    });
    if (hint) {
      hint.textContent = j.configured
        ? `${cfg.label} is configured and ready.`
        : `Save an API key and base URL to enable ${cfg.label}.`;
    }
  }

  async function saveAiSimpleLlmSettings(product) {
    const cfg = AI_SIMPLE_LLM[product];
    if (!cfg) return;
    const apiKey = (document.getElementById(`${cfg.prefix}-api-key`)?.value || "").trim();
    const baseUrl = (document.getElementById(`${cfg.prefix}-base-url`)?.value || "").trim();
    const model = (document.getElementById(`${cfg.prefix}-model`)?.value || "").trim();
    const skipTls = !!document.getElementById(`${cfg.prefix}-skip-tls`)?.checked;
    setStatus(cfg.statusId, "Saving…");
    const body = {
      base_url: baseUrl,
      model: model || "gpt-4o-mini",
      skip_tls_verify: skipTls,
      ...readAiTempMaxBody(cfg.prefix, {
        temperature: cfg.defaultTemperature,
        maxTokens: cfg.defaultMaxTokens,
      }),
    };
    if (apiKey) body.api_key = apiKey;
    const r = await api(cfg.apiPath, { method: "PUT", body: JSON.stringify(body) });
    const j = await r.json().catch(() => ({}));
    if (!r.ok) {
      setStatus(cfg.statusId, j.error || "Save failed.");
      return;
    }
    const keyEl = document.getElementById(`${cfg.prefix}-api-key`);
    if (keyEl) keyEl.value = "";
    setStatus(cfg.statusId, "Saved.");
    await loadAiSimpleLlmSettings(product);
  }

  function adsNum(id, fallback) {
    const el = document.getElementById(id);
    if (!el) return fallback;
  ...
  function adsUpdateScopeVisibility() {
    const mode = document.getElementById("ads-scope-mode")?.value || "all";
    const wrap = document.getElementById("ads-folder-scope-wrap");
    if (wrap) wrap.hidden = mode !== "folders";
  }

  function adsRenderFolderChips() {
    const chips = document.getElementById("ads-folder-chips");
    if (!chips) return;
    chips.innerHTML = "";
    for (const [id, meta] of adsSelectedFolders) {
      const btn = document.createElement("button");
      btn.type = "button";
      btn.className = "nc-btn nc-btn-secondary";
      btn.style.fontSize = "0.82rem";
      btn.textContent = `${meta.path || meta.name || id} ×`;
      btn.addEventListener("click", () => {
        adsSelectedFolders.delete(id);
        adsRenderFolderChips();
      });
      chips.appendChild(btn);
    }
  }

  function adsAddFolderOption(folder) {
    if (!folder || !folder.id) return;
    adsSelectedFolders.set(folder.id, { id: folder.id, path: folder.path, name: folder.name });
    adsRenderFolderChips();
  }

  function adsRenderFolderPickList(folders) {
    const list = document.getElementById("ads-folder-pick-list");
    if (!list) return;
    list.innerHTML = "";
    ...
    for (const f of folders) {
      ...
      add.type = "button";
      add.className = "nc-btn nc-btn-secondary";
      add.style.fontSize = "0.8rem";
      add.style.marginLeft = "0.35rem";
      add.textContent = adsSelectedFolders.has(f.id) ? "Added" : "Add";
      add.disabled = adsSelectedFolders.has(f.id);
      add.addEventListener("click", () => {
        adsAddFolderOption(f);
        add.textContent = "Added";
        add.disabled = true;
      });
      li.appendChild(document.createTextNode(f.path || f.name || String(f.id)));
      li.appendChild(add);
      list.appendChild(li);
    }
  }

  async function adsLoadFolderPicker(parentId, q) {
    const params = new URLSearchParams();
    if (q) params.set("q", q);
    else if (parentId != null) params.set("parent_id", String(parentId));
    const qs = params.toString();
    const r = await api(`/api/ai-document-search/index-folders${qs ? `?${qs}` : ""}`, { method: "GET" });
    const j = await r.json().catch(() => ({}));
    if (!r.ok) {
      setStatus("admin-ads-status", j.error || "Could not load folders.");
      return;
    }
    adsRenderFolderPickList(j.folders || []);
  }

  async function loadAiDocumentSearchSettings() {
    const r = await api("/api/settings/ai-document-search", { method: "GET" });
    const j = await r.json().catch(() => ({}));
    const base = document.getElementById("ads-base-url");
    const model = document.getElementById("ads-model");
    const embModel = document.getElementById("ads-embedding-model");
    const skipTls = document.getElementById("ads-skip-tls");
    const hint = document.getElementById("admin-ads-config-hint");
    if (!r.ok) {
      setStatus("admin-ads-status", j.error || "Could not load AI Document Search settings.");
      return;
    }
    aiSettingsApiKeys.document_search = !!j.api_key_set;
    aiSettingsApiKeys.chatbot = !!j.peer_api_key_set;
    updateAiCopyKeyButtons();
    const adsKey = document.getElementById("ads-api-key");
    if (adsKey && j.api_key_set) {
      adsKey.placeholder = "Saved key on server — leave blank to keep, or paste to replace";
    } else if (adsKey) {
      adsKey.placeholder = "sk-… or leave blank to keep saved key";
    }
    if (base) base.value = j.base_url || "";
    if (model) model.value = j.model || "gpt-4o-mini";
    applyAiTempMaxFields("ads", j, { temperature: 0.35, maxTokens: 1800 });
    if (embModel) embModel.value = j.embedding_model || "text-embedding-3-small";
    if (skipTls) skipTls.checked = !!j.skip_tls_verify;
    const chunkChars = document.getElementById("ads-chunk-chars");
    const maxChunks = document.getElementById("ads-max-chunks");
    const maxFileChars = document.getElementById("ads-max-file-chars");
    const embedBatch = document.getElementById("ads-embed-batch");
    const searchTopK = document.getElementById("ads-search-top-k");
    const indexBatch = document.getElementById("ads-index-batch");
    const scopeMode = document.getElementById("ads-scope-mode");
    const includeGlob = document.getElementById("ads-include-glob");
    const excludeGlob = document.getElementById("ads-exclude-glob");
    if (chunkChars) chunkChars.value = j.chunk_chars ?? 2000;
    if (maxChunks) maxChunks.value = j.max_chunks_per_file ?? 150;
    if (maxFileChars) maxFileChars.value = j.max_file_chars ?? 600000;
    if (embedBatch) embedBatch.value = j.embed_batch_size ?? 48;
    if (searchTopK) searchTopK.value = j.search_top_k ?? 18;
    if (indexBatch) indexBatch.value = j.index_batch_files ?? 120;
    if (scopeMode) scopeMode.value = j.index_scope_mode === "folders" ? "folders" : "all";
    if (includeGlob) includeGlob.value = j.index_include_glob || "";
    if (excludeGlob) excludeGlob.value = j.index_exclude_glob || "";
    adsSelectedFolders.clear();
    for (const f of j.index_folders || []) {
      adsSelectedFolders.set(f.id, f);
    }
    adsRenderFolderChips();
    adsUpdateScopeVisibility();
    if (hint) {
      const scope =
        j.index_scope_mode === "folders"
          ? `Indexing limited to ${(j.index_folders || []).length} folder(s).`
          : "Indexing all supported files (with optional name filters).";
      hint.textContent = j.configured
        ? `AI is configured and ready. ${scope}`
        : "Save an API key to enable chat answers.";
    }
  }

  document.getElementById("ads-scope-mode")?.addEventListener("change", adsUpdateScopeVisibility);
  document.getElementById("ads-folder-search-btn")?.addEventListener("click", () => {
    const q = (document.getElementById("ads-folder-search")?.value || "").trim();
    adsLoadFolderPicker(null, q);
  });
  document.getElementById("ads-folder-root-btn")?.addEventListener("click", () => {
    const search = document.getElementById("ads-folder-search");
    if (search) search.value = "";
    adsLoadFolderPicker(null, "");
  });

  document.getElementById("ads-save")?.addEventListener("click", async () => {
    const apiKey = (document.getElementById("ads-api-key")?.value || "").trim();
    const baseUrl = (document.getElementById("ads-base-url")?.value || "").trim();
    const model = (document.getElementById("ads-model")?.value || "").trim();
    const embeddingModel = (document.getElementById("ads-embedding-model")?.value || "").trim();
    const skipTls = !!(document.getElementById("ads-skip-tls") && document.getElementById("ads-skip-tls").checked);
    const scopeMode = document.getElementById("ads-scope-mode")?.value || "all";
    setStatus("admin-ads-status", "Saving…");
    const body = {
      base_url: baseUrl,
      model: model || "gpt-4o-mini",
      embedding_model: embeddingModel || "text-embedding-3-small",
      skip_tls_verify: skipTls,
      ...readAiTempMaxBody("ads", { temperature: 0.35, maxTokens: 1800 }),
      chunk_chars: adsNum("ads-chunk-chars", 2000),
      max_chunks_per_file: adsNum("ads-max-chunks", 150),
      max_file_chars: adsNum("ads-max-file-chars", 600000),
      embed_batch_size: adsNum("ads-embed-batch", 48),
      search_top_k: adsNum("ads-search-top-k", 18),
      index_batch_files: adsNum("ads-index-batch", 120),
      index_scope_mode: scopeMode,
      index_folder_ids: [...adsSelectedFolders.keys()],
      index_include_glob: (document.getElementById("ads-include-glob")?.value || "").trim(),
      index_exclude_glob: (document.getElementById("ads-exclude-glob")?.value || "").trim(),
    };
    if (apiKey) body.api_key = apiKey;
    const r = await api("/api/settings/ai-document-search", {
      method: "PUT",
      body: JSON.stringify(body),
    });
    const j = await r.json().catch(() => ({}));
    if (!r.ok) {
      setStatus("admin-ads-status", j.error || "Save failed.");
      return;
    }
    const keyEl = document.getElementById("ads-api-key");
    if (keyEl) keyEl.value = "";
    const purged = j.purged_chunks ? ` Removed ${j.purged_chunks} out-of-scope chunk(s).` : "";
    setStatus("admin-ads-status", `Saved.${purged}`);
    loadAiDocumentSearchSettings();
  });

  function showAiSettingsPane(which) {
    const active = AI_SETTINGS_PANES.includes(which) ? which : "document_search";
    for (const pane of AI_SETTINGS_PANES) {
      const panel = document.getElementById(`admin-ai-settings-pane-${pane}`);
      const tab = document.querySelector(`[data-ai-settings-pane="${pane}"]`);
  ...
  async function loadAiChatbotSettings() {
    const r = await api("/api/settings/ai-chatbot", { method: "GET" });
    const j = await r.json().catch(() => ({}));
    const hint = document.getElementById("admin-aichat-config-hint");
    if (!r.ok) {
      setStatus("admin-aichat-status", j.error || "Could not load AI Chatbot settings.");
      return;
    }
    aiSettingsApiKeys.chatbot = !!j.api_key_set;
    aiSettingsApiKeys.document_search = !!j.peer_api_key_set;
    updateAiCopyKeyButtons();
    const chatKey = document.getElementById("aichat-api-key");
    if (chatKey && j.api_key_set) {
      chatKey.placeholder = "Saved key on server — leave blank to keep, or paste to replace";
    } else if (chatKey) {
      chatKey.placeholder = "sk-… or leave blank to keep saved key";
    }
    const base = document.getElementById("aichat-base-url");
    const model = document.getElementById("aichat-model");
    const prompt = document.getElementById("aichat-system-prompt");
    const temp = document.getElementById("aichat-temperature");
    const maxTok = document.getElementById("aichat-max-tokens");
    const skipTls = document.getElementById("aichat-skip-tls");
    if (base) base.value = j.base_url || "";
    if (model) model.value = j.model || "gpt-4o-mini";
    if (prompt) prompt.value = j.system_prompt || "";
    if (temp) temp.value = j.temperature ?? 0.6;
    if (maxTok) maxTok.value = j.max_tokens ?? 2000;
    if (skipTls) skipTls.checked = !!j.skip_tls_verify;
    if (hint) {
      hint.textContent = j.configured
        ? "Chatbot is configured and ready."
        : "Save an API key to enable the AI Chatbot module.";
    }
  }

  async function loadAiSettings() {
    showAiSettingsPane("document_search");
    await Promise.all([
      loadAiDocumentSearchSettings(),
      loadAiChatbotSettings(),
      loadAiSimpleLlmSettings("policy_assistant"),
      loadAiSimpleLlmSettings("cv_builder"),
      loadAiSimpleLlmSettings("tender_assistant"),
    ]);
    updateAiCopyKeyButtons();
  }

  document.getElementById("admin-ai-settings-subnav")?.addEventListener("click", (e) => {
    const btn = e.target.closest("[data-ai-settings-pane]");
    if (!btn || btn.disabled) return;
    e.preventDefault();
    showAiSettingsPane(btn.getAttribute("data-ai-settings-pane") || "document_search");
  });

  document.getElementById("ads-copy-api-key")?.addEventListener("click", () => {
    void copyAiApiKey("document_search", "chatbot");
  });
  document.getElementById("aichat-copy-api-key")?.addEventListener("click", () => {
    void copyAiApiKey("chatbot", "document_search");
  });
  document.querySelectorAll(".ai-copy-api-key-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      const to = btn.getAttribute("data-copy-to");
      const from = btn.getAttribute("data-copy-from") || "document_search";
      if (to) void copyAiApiKey(to, from);
    });
  });
  document.getElementById("aipolicy-save")?.addEventListener("click", () => {
    void saveAiSimpleLlmSettings("policy_assistant");
  });
  document.getElementById("aicv-save")?.addEventListener("click", () => {
    void saveAiSimpleLlmSettings("cv_builder");
  });
  document.getElementById("aitender-save")?.addEventListener("click", () => {
    void saveAiSimpleLlmSettings("tender_assistant");
  });

  document.getElementById("aichat-save")?.addEventListener("click", async () => {
    const apiKey = (document.getElementById("aichat-api-key")?.value || "").trim();
    const baseUrl = (document.getElementById("aichat-base-url")?.value || "").trim();
    const model = (document.getElementById("aichat-model")?.value || "").trim();
    const systemPrompt = (document.getElementById("aichat-system-prompt")?.value || "").trim();
    const skipTls = !!(document.getElementById("aichat-skip-tls") && document.getElementById("aichat-skip-tls").checked);
    setStatus("admin-aichat-status", "Saving…");
    const body = {
      base_url: baseUrl,
      model: model || "gpt-4o-mini",
      system_prompt: systemPrompt,
      skip_tls_verify: skipTls,
      temperature: parseFloat(document.getElementById("aichat-temperature")?.value || "0.6"),
      max_tokens: parseInt(document.getElementById("aichat-max-tokens")?.value || "2000", 10),
    };
    if (apiKey) body.api_key = apiKey;
    const r = await api("/api/settings/ai-chatbot", {
      method: "PUT",
      body: JSON.stringify(body),
    });
    const j = await r.json().catch(() => ({}));
    if (!r.ok) {
      setStatus("admin-aichat-status", j.error || "Save failed.");
      return;
    }
    const keyEl = document.getElementById("aichat-api-key");
    if (keyEl) keyEl.value = "";
    setStatus("admin-aichat-status", "Saved.");
    loadAiChatbotSettings();
  });
})();
