(function () {
  ...
  function setActiveTab(name, { persist = true } = {}) {
    ...
    setHidden("admin-tab-email_notifications", chosen === "email_notifications");
    setHidden("admin-tab-sip", chosen === "sip");
    setHidden("admin-tab-intranet_signaling", chosen === "intranet_signaling");
    ...
    if (chosen === "email_notifications") loadEmailNotificationSettings();
    if (chosen === "sip") loadSipSettings();
    if (chosen === "intranet_signaling") loadIntranetSignalingSettings();
  ...
  const ACCESS_MATRIX_SECTIONS = [
    ...
    },
    {
      ...
      items: [
        ...
        ["kanban.edit", "Edit KanBan (rename / delete columns)"],
  ...
  async function loadSoftwareVersion() {
    ...
    const deployedEl = document.getElementById("sw-display-deployed");
    if (deployedEl) {
      const ts = fmtDeployTs(j.current_deployed_at);
      deployedEl.textContent = ts && ts !== "—" ? `Last deployed ${ts}` : "";
      deployedEl.style.display = deployedEl.textContent ? "block" : "none";
    }
    ...
    const bits = [];
    if (!j.is_git_repo)
      bits.push("This path is not a Git clone — deploy a Git checkout for in-app Git-based updates.");
    else if (j.is_package_deploy)
      bits.push("Recorded deployment was from a legacy package upgrade.");
    ...
    if (pc) pc.textContent = j.previous_version || "—";
    if (cc) cc.textContent = j.current_version || "—";
  ...
  function updateDocumentEditorCards(_provider) {
    const ooCard = document.getElementById("integrations-onlyoffice-card");
    if (ooCard) ooCard.style.opacity = "1";
  }
  ...
  async function loadSipSettings() {
    const r = await api("/api/settings/sip", { method: "GET" });
    const j = await r.json().catch(() => ({}));
    const hint = document.getElementById("admin-sip-config-hint");
    if (!r.ok) {
      setStatus("admin-sip-status", j.error || "Could not load SIP settings.");
      return;
    }
    const set = (id, val) => {
      const el = document.getElementById(id);
    ...
    setCheck("sip-enabled", j.enabled);
    set("sip-domain", j.sip_domain || "");
    set("sip-server-host", j.sip_server_host || "");
    set("sip-server-port", j.sip_server_port != null ? String(j.sip_server_port) : "5060");
    set("sip-websocket-uri", j.websocket_uri || "");
    set("sip-outbound-proxy", j.outbound_proxy || "");
    set("sip-registration-expires", j.registration_expires != null ? String(j.registration_expires) : "300");
    set("sip-stun-server", j.stun_server || "");
    set("sip-dtmf-mode", j.dtmf_mode || "info");
    setCheck("sip-use-session-timers", j.use_session_timers !== false);
    setCheck("sip-allow-insecure-ws", j.allow_insecure_websocket);
    if (hint) {
      hint.textContent = j.configured
        ? "SIP phone is configured. Users will see Phone under About Company."
        : "Enable SIP and save domain + WebSocket URI to activate the phone.";
    }
  }

  function readSipForm() {
    return {
      enabled: !!(document.getElementById("sip-enabled") && document.getElementById("sip-enabled").checked),
      sip_domain: (document.getElementById("sip-domain")?.value || "").trim(),
      sip_server_host: (document.getElementById("sip-server-host")?.value || "").trim(),
      sip_server_port: parseInt(document.getElementById("sip-server-port")?.value || "5060", 10),
      websocket_uri: (document.getElementById("sip-websocket-uri")?.value || "").trim(),
      outbound_proxy: (document.getElementById("sip-outbound-proxy")?.value || "").trim(),
      registration_expires: parseInt(document.getElementById("sip-registration-expires")?.value || "300", 10),
      stun_server: (document.getElementById("sip-stun-server")?.value || "").trim(),
      dtmf_mode: document.getElementById("sip-dtmf-mode")?.value || "info",
      use_session_timers: !!(document.getElementById("sip-use-session-timers") && document.getElementById("sip-use-session-timers").checked),
      allow_insecure_websocket: !!(document.getElementById("sip-allow-insecure-ws") && document.getElementById("sip-allow-insecure-ws").checked),
    };
  }
  ...
  const EMAIL_NOTIF_PANE_KEY = "admin-email-notif-pane";
  let emailNotifData = null;

  function setEmailNotifStatus(msg) {
    const el = document.getElementById("admin-email-notif-status");
    if (el) el.textContent = msg || "";
  }

  function showEmailNotifPane(moduleKey, { persist = true } = {}) {
    const chosen = String(moduleKey || "").trim();
    document.querySelectorAll("#admin-email-notif-subnav .nc-admin-ai-settings-tab").forEach((btn) => {
      const on = btn.dataset.emailNotifModule === chosen;
      btn.classList.toggle("is-active", on);
      btn.setAttribute("aria-selected", String(on));
    });
    document.querySelectorAll(".nc-admin-email-notif-pane").forEach((pane) => {
      pane.hidden = pane.dataset.emailNotifModule !== chosen;
    });
    if (persist) {
      try {
        window.localStorage.setItem(EMAIL_NOTIF_PANE_KEY, chosen);
      } catch (_e) {}
    }
  }

  function renderEmailNotificationPanes(data) {
    const subnav = document.getElementById("admin-email-notif-subnav");
    const panesRoot = document.getElementById("admin-email-notif-panes");
    if (!subnav || !panesRoot) return;
    const order = Array.isArray(data.module_order) ? data.module_order : Object.keys(data.modules || {});
    ...
    order.forEach((key, idx) => {
      ...
      tab.type = "button";
      tab.className = `nc-admin-ai-settings-tab${idx === 0 ? " is-active" : ""}`;
      tab.dataset.emailNotifModule = key;
      tab.setAttribute("role", "tab");
      tab.setAttribute("aria-selected", idx === 0 ? "true" : "false");
      tab.textContent = mod.label || key;
      subnav.appendChild(tab);

      const pane = document.createElement("div");
      pane.className = "nc-admin-email-notif-pane nc-admin-section-body";
      pane.dataset.emailNotifModule = key;
      pane.hidden = idx !== 0;
      pane.setAttribute("role", "tabpanel");

      const reminderDaysVal = Array.isArray(mod.reminder_days) ? mod.reminder_days.join(", ") : "1, 7";
      const scheduleFields = mod.instant_only
        ? `<p class="nc-detail-muted" style="margin:0 0 0.75rem;">Sent immediately when a watched wiki page is updated (not on a schedule).</p>`
        : `
        ${mod.has_send_hour ? `
          <div class="nc-share-add-row" style="margin-bottom:0.75rem;align-items:flex-end;gap:0.5rem;">
            <label class="nc-detail-label" for="en-${key}-hour" style="margin:0;">Send at (local time)</label>
            <select id="en-${key}-hour" class="nc-detail-input nc-en-hour" data-module="${key}" style="flex:0 0 8rem;"></select>
          </div>` : ""}
        ${mod.has_reminder_days ? `
          <label class="nc-detail-label" for="en-${key}-days">Days before (comma-separated)</label>
          <input type="text" id="en-${key}-days" class="nc-detail-input nc-en-days" data-module="${key}" value="${esc(reminderDaysVal)}" placeholder="30, 7, 1" style="width:100%;max-width:16rem;margin-bottom:0.75rem;">` : ""}
        ${mod.has_interval_days ? `
          <label class="nc-detail-label" for="en-${key}-interval">Reminder interval (days)</label>
          <input type="number" id="en-${key}-interval" class="nc-detail-input nc-en-interval" data-module="${key}" min="1" max="90" value="${Number(mod.interval_days) || 7}" style="flex:0 0 6rem;margin-bottom:0.75rem;">` : ""}
        ${mod.has_stale_days ? `
          <label class="nc-detail-label" for="en-${key}-stale">No activity for (days)</label>
          <input type="number" id="en-${key}-stale" class="nc-detail-input nc-en-stale" data-module="${key}" min="1" max="365" value="${Number(mod.stale_days) || 14}" style="flex:0 0 6rem;margin-bottom:0.75rem;">` : ""}
        ${!mod.instant_only ? `
          <button type="button" class="nc-btn nc-btn-secondary nc-en-send-now" data-module="${key}" style="margin-bottom:0.85rem;">Send reminders now</button>` : ""}`;

      pane.innerHTML = `
        <section class="nc-admin-card" aria-labelledby="en-heading-${key}">
          <h3 id="en-heading-${key}" class="nc-admin-subhead" style="margin-top:0;">${key === "workforce" ? "Workforce managers" : esc(mod.label || key)}</h3>
          <p class="nc-detail-muted" style="margin:0 0 0.75rem;">${esc(mod.description || "")}</p>
          <label class="nc-auth-remember" style="display:flex;align-items:flex-start;gap:0.45rem;margin-bottom:0.75rem;">
            <input type="checkbox" class="nc-en-enabled" data-module="${key}" ${mod.enabled ? "checked" : ""} style="margin-top:0.15rem;">
            <span><strong>Enable email reminders</strong>${key === "workforce" ? " for managers" : ""}</span>
          </label>
          ${scheduleFields}
          <label class="nc-detail-label" for="en-${key}-subject">Email subject</label>
          <input type="text" id="en-${key}-subject" class="nc-detail-input nc-en-subject" data-module="${key}" value="${esc(mod.subject || "")}" style="width:100%;max-width:36rem;" autocomplete="off">
          <label class="nc-detail-label" for="en-${key}-body" style="margin-top:0.55rem;">Email body</label>
          <textarea id="en-${key}-body" class="nc-detail-input nc-en-body" data-module="${key}" rows="8" style="width:100%;max-width:36rem;resize:vertical;font-family:inherit;">${esc(mod.body || "")}</textarea>
          <p class="nc-detail-muted nc-en-placeholders" data-module="${key}" style="margin:0.65rem 0 0.85rem;font-size:0.82rem;">Placeholders: ${(mod.placeholders || []).map((p) => esc(p)).join(", ")}</p>
          <div class="nc-share-add-row" style="flex-wrap:wrap;align-items:flex-end;gap:0.5rem;">
            <button type="button" class="nc-btn nc-btn-primary nc-en-save" data-module="${key}">Save</button>
            <label class="nc-detail-label" for="en-${key}-test-to" style="margin:0;">Test recipient</label>
            <input type="email" id="en-${key}-test-to" class="nc-detail-input nc-en-test-to" data-module="${key}" placeholder="you@example.com" style="flex:1 1 12rem;max-width:16rem;" autocomplete="off">
            <button type="button" class="nc-btn nc-btn-secondary nc-en-test" data-module="${key}">Send test email</button>
          </div>
        </section>`;

      if (key === "workforce" && mod.has_resource_section) {
        const resourceDaysVal = Array.isArray(mod.resource_reminder_days)
          ? mod.resource_reminder_days.join(", ")
          : "30, 7, 1";
        const resourceSection = document.createElement("section");
        resourceSection.className = "nc-admin-card";
        resourceSection.style.marginTop = "1rem";
        resourceSection.setAttribute("aria-labelledby", "en-heading-workforce-resource");
        resourceSection.innerHTML = `
          <h3 id="en-heading-workforce-resource" class="nc-admin-subhead" style="margin-top:0;">Contractor resources</h3>
          <p class="nc-detail-muted" style="margin:0 0 0.75rem;">Email each contractor (resource) linked to a company when that company's PI/PL Insurance or Workcover document is approaching expiry.</p>
          <label class="nc-auth-remember" style="display:flex;align-items:flex-start;gap:0.45rem;margin-bottom:0.75rem;">
            <input type="checkbox" class="nc-en-resource-enabled" data-module="workforce" ${mod.resource_enabled ? "checked" : ""} style="margin-top:0.15rem;">
            <span><strong>Enable email reminders for resources</strong></span>
          </label>
          <label class="nc-detail-label" for="en-workforce-resource-days">Days before (comma-separated)</label>
          <input type="text" id="en-workforce-resource-days" class="nc-detail-input nc-en-resource-days" data-module="workforce" value="${esc(resourceDaysVal)}" placeholder="30, 7, 1" style="width:100%;max-width:16rem;margin-bottom:0.75rem;">
          <p class="nc-detail-muted" style="margin:0 0 0.75rem;font-size:0.88rem;">Uses the same send time as manager reminders above. Reminders are sent for PI/PL Insurance and Workcover separately.</p>
          <label class="nc-detail-label" for="en-workforce-resource-subject">Email subject</label>
          <input type="text" id="en-workforce-resource-subject" class="nc-detail-input nc-en-resource-subject" data-module="workforce" value="${esc(mod.resource_subject || "")}" style="width:100%;max-width:36rem;" autocomplete="off">
          <label class="nc-detail-label" for="en-workforce-resource-body" style="margin-top:0.55rem;">Email body</label>
          <textarea id="en-workforce-resource-body" class="nc-detail-input nc-en-resource-body" data-module="workforce" rows="8" style="width:100%;max-width:36rem;resize:vertical;font-family:inherit;">${esc(mod.resource_body || "")}</textarea>
          <p class="nc-detail-muted" style="margin:0.65rem 0 0.85rem;font-size:0.82rem;">Placeholders: ${(mod.resource_placeholders || []).map((p) => esc(p)).join(", ")}</p>
          <div class="nc-share-add-row" style="flex-wrap:wrap;align-items:flex-end;gap:0.5rem;">
            <label class="nc-detail-label" for="en-workforce-resource-test-to" style="margin:0;">Test recipient</label>
            <input type="email" id="en-workforce-resource-test-to" class="nc-detail-input nc-en-resource-test-to" data-module="workforce" placeholder="contractor@example.com" style="flex:1 1 12rem;max-width:16rem;" autocomplete="off">
            <button type="button" class="nc-btn nc-btn-secondary nc-en-test-resource" data-module="workforce">Send resource test email</button>
          </div>`;
        pane.appendChild(resourceSection);
      }
      panesRoot.appendChild(pane);

      const hourSel = pane.querySelector(".nc-en-hour");
      if (hourSel) {
        for (let h = 0; h < 24; h += 1) {
          ...
          if (h === Number(mod.send_hour_local ?? 9)) opt.selected = true;
          hourSel.appendChild(opt);
        }
      }
    });

    const emailNotifRoot = document.getElementById("admin-tab-email_notifications");
    if (emailNotifRoot && !emailNotifRoot.dataset.emailNotifWired) {
      emailNotifRoot.dataset.emailNotifWired = "1";
      emailNotifRoot.addEventListener("click", (e) => {
        const tabBtn = e.target.closest("[data-email-notif-module].nc-admin-ai-settings-tab");
        if (tabBtn) showEmailNotifPane(tabBtn.dataset.emailNotifModule);
      });
      emailNotifRoot.addEventListener("click", async (e) => {
        const saveBtn = e.target.closest(".nc-en-save");
        const testBtn = e.target.closest(".nc-en-test");
        const testResourceBtn = e.target.closest(".nc-en-test-resource");
        const sendBtn = e.target.closest(".nc-en-send-now");
        const modKey = (saveBtn || testBtn || testResourceBtn || sendBtn)?.dataset.module;
        if (!modKey) return;
        if (saveBtn) await saveEmailNotificationModule(modKey);
        else if (testBtn) await testEmailNotificationModule(modKey);
        else if (testResourceBtn) await testEmailNotificationModule(modKey, { variant: "resource" });
        else if (sendBtn) await sendEmailNotificationNow(modKey);
      });
    }
  }

  function readEmailNotificationModuleForm(moduleKey) {
    const key = String(moduleKey || "").trim();
    const enabled = !!document.querySelector(`.nc-en-enabled[data-module="${key}"]`)?.checked;
    const subject = (document.querySelector(`.nc-en-subject[data-module="${key}"]`)?.value || "").trim();
    const body = (document.querySelector(`.nc-en-body[data-module="${key}"]`)?.value || "").trim();
    const out = { enabled, subject, body };
    const hourEl = document.querySelector(`.nc-en-hour[data-module="${key}"]`);
    if (hourEl) out.send_hour_local = Number(hourEl.value);
    const daysEl = document.querySelector(`.nc-en-days[data-module="${key}"]`);
    if (daysEl) {
      out.reminder_days = daysEl.value
        .split(/[,;\s]+/)
        .map((s) => Number(s.trim()))
        .filter((n) => Number.isFinite(n) && n > 0);
    }
    const intervalEl = document.querySelector(`.nc-en-interval[data-module="${key}"]`);
    if (intervalEl) out.interval_days = Number(intervalEl.value);
    const staleEl = document.querySelector(`.nc-en-stale[data-module="${key}"]`);
    if (staleEl) out.stale_days = Number(staleEl.value);
    if (key === "workforce") {
      const resourceEnabledEl = document.querySelector(`.nc-en-resource-enabled[data-module="${key}"]`);
      if (resourceEnabledEl) out.resource_enabled = !!resourceEnabledEl.checked;
      const resourceDaysEl = document.querySelector(`.nc-en-resource-days[data-module="${key}"]`);
      if (resourceDaysEl) {
        out.resource_reminder_days = resourceDaysEl.value
          .split(/[,;\s]+/)
          .map((s) => Number(s.trim()))
          .filter((n) => Number.isFinite(n) && n > 0);
      }
      const resourceSubjectEl = document.querySelector(`.nc-en-resource-subject[data-module="${key}"]`);
      if (resourceSubjectEl) out.resource_subject = (resourceSubjectEl.value || "").trim();
      const resourceBodyEl = document.querySelector(`.nc-en-resource-body[data-module="${key}"]`);
      if (resourceBodyEl) out.resource_body = (resourceBodyEl.value || "").trim();
    }
    return out;
  }

  async function saveEmailNotificationModule(moduleKey) {
    setEmailNotifStatus("Saving…");
    const settings = readEmailNotificationModuleForm(moduleKey);
    const r = await api("/api/settings/email-notifications", {
      method: "PUT",
      body: JSON.stringify({ modules: { [moduleKey]: settings } }),
    });
    const j = await r.json().catch(() => ({}));
    if (!r.ok) {
      setEmailNotifStatus(j.error || "Save failed.");
      return;
    }
    emailNotifData = j;
    setEmailNotifStatus("Saved.");
  }

  async function testEmailNotificationModule(moduleKey, { variant = "" } = {}) {
    const isResource = variant === "resource";
    const toSel = isResource
      ? `.nc-en-resource-test-to[data-module="${moduleKey}"]`
      : `.nc-en-test-to[data-module="${moduleKey}"]`;
    const to = (document.querySelector(toSel)?.value || "").trim();
    if (!to) {
      setEmailNotifStatus("Enter a test recipient email.");
      return;
    }
    setEmailNotifStatus("Sending test email…");
    await saveEmailNotificationModule(moduleKey);
    const r = await api("/api/settings/email-notifications/test", {
      method: "POST",
      body: JSON.stringify({ module: moduleKey, to, variant: isResource ? "resource" : "" }),
    });
    const j = await r.json().catch(() => ({}));
    setEmailNotifStatus(r.ok && j.ok ? j.message || "Test email sent." : j.error || "Test failed.");
  }

  async function sendEmailNotificationNow(moduleKey) {
    setEmailNotifStatus("Sending reminders…");
    await saveEmailNotificationModule(moduleKey);
    const r = await api("/api/settings/email-notifications/send-now", {
      method: "POST",
      body: JSON.stringify({ module: moduleKey }),
    });
    const j = await r.json().catch(() => ({}));
    if (!r.ok) {
      setEmailNotifStatus(j.error || j.skipped || "Send failed.");
      return;
    }
    const sent = Number(j.sent) || 0;
    const failed = Number(j.failed) || 0;
    const skipped = j.skipped ? ` (${j.skipped})` : "";
    setEmailNotifStatus(`Done${skipped}: ${sent} sent, ${failed} failed.`);
  }

  async function loadEmailNotificationSettings() {
    if (!document.getElementById("admin-tab-email_notifications")) return;
    setEmailNotifStatus("Loading…");
    const r = await api("/api/settings/email-notifications", { method: "GET" });
    const j = await r.json().catch(() => ({}));
    if (!r.ok) {
      setEmailNotifStatus(j.error || "Failed to load settings.");
      return;
    }
    emailNotifData = j;
    renderEmailNotificationPanes(j);
    let pane = (j.module_order || [])[0] || "events";
    try {
      const saved = window.localStorage.getItem(EMAIL_NOTIF_PANE_KEY);
      if (saved && (j.module_order || []).includes(saved)) pane = saved;
    } catch (_e) {}
    showEmailNotifPane(pane, { persist: false });
    setEmailNotifStatus("");
  }
  ...
  if (sipSave) {
    sipSave.addEventListener("click", async () => {
      setStatus("admin-sip-status", "Saving…");
      const r = await api("/api/settings/sip", {
        method: "PUT",
        body: JSON.stringify(readSipForm()),
      });
      const j = await r.json().catch(() => ({}));
      setStatus("admin-sip-status", r.ok ? "Saved." : j.error || "Save failed");
      if (r.ok) await loadSipSettings();
    });
  }

  const intranetSignalingSave = document.getElementById("intranet-signaling-save");
  if (intranetSignalingSave) {
    intranetSignalingSave.addEventListener("click", async () => {
      setStatus("admin-intranet-signaling-status", "Saving…");
      const r = await api("/api/settings/intranet-signaling", {
        method: "PUT",
        body: JSON.stringify(readIntranetSignalingForm()),
      });
      const j = await r.json().catch(() => ({}));
      setStatus("admin-intranet-signaling-status", r.ok ? "Saved." : j.error || "Save failed");
      if (r.ok) await loadIntranetSignalingSettings();
    });
  }

  async function loadIntranetSignalingSettings() {
    const r = await api("/api/settings/intranet-signaling", { method: "GET" });
    const j = await r.json().catch(() => ({}));
    const hint = document.getElementById("admin-intranet-signaling-hint");
    if (!r.ok) {
      setStatus("admin-intranet-signaling-status", j.error || "Could not load signaling settings.");
      return;
    }
    const set = (id, val) => {
      const el = document.getElementById(id);
    ...
    setCheck("intranet-signaling-enabled", j.enabled);
    setCheck("intranet-signaling-voice", j.voice_enabled !== false);
    setCheck("intranet-signaling-video", j.video_enabled !== false);
    set("intranet-signaling-max", j.max_participants != null ? String(j.max_participants) : "40");
    set("intranet-signaling-stun", j.stun_server || "");
    set("intranet-signaling-turn", j.turn_server || "");
    set("intranet-signaling-turn-user", j.turn_username || "");
    const pwEl = document.getElementById("intranet-signaling-turn-password");
    if (pwEl) pwEl.value = "";
    setCheck("intranet-signaling-use-socket", j.use_socket_signaling !== false);
    setCheck("intranet-signaling-http-fallback", j.allow_http_signaling_fallback !== false);
    if (hint) {
      hint.textContent = j.configured
        ? "Intranet signaling is active. Team Chat voice/video uses this server."
        : "Enable signaling and save to activate Team Chat WebRTC calls.";
    }
  }

  function readIntranetSignalingForm() {
    return {
      enabled: !!(
        document.getElementById("intranet-signaling-enabled") &&
        document.getElementById("intranet-signaling-enabled").checked
      ),
      voice_enabled: !!(
        document.getElementById("intranet-signaling-voice") &&
        document.getElementById("intranet-signaling-voice").checked
      ),
      video_enabled: !!(
        document.getElementById("intranet-signaling-video") &&
        document.getElementById("intranet-signaling-video").checked
      ),
      max_participants: parseInt(document.getElementById("intranet-signaling-max")?.value || "40", 10),
      stun_server: (document.getElementById("intranet-signaling-stun")?.value || "").trim(),
      turn_server: (document.getElementById("intranet-signaling-turn")?.value || "").trim(),
      turn_username: (document.getElementById("intranet-signaling-turn-user")?.value || "").trim(),
      turn_password: (document.getElementById("intranet-signaling-turn-password")?.value || "").trim(),
      use_socket_signaling: !!(
        document.getElementById("intranet-signaling-use-socket") &&
        document.getElementById("intranet-signaling-use-socket").checked
      ),
      allow_http_signaling_fallback: !!(
        document.getElementById("intranet-signaling-http-fallback") &&
        document.getElementById("intranet-signaling-http-fallback").checked
      ),
    };
  }
  ...
  (async () => {
    ...
    setActiveTab(urlTab || savedTab || defaultTab, { persist: false });
  })();
