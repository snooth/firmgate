(function () {
  ...
  function setActiveTab(name, { persist = true } = {}) {
    ...
    if (chosen === "modules") loadModulesSettings();
  ...
  async function saveModulesSettings() {
    ...
    modulesState = jj;
    renderModules();
    setStatus("admin-modules-status", "Modules saved.");
  }
  ...
  if (portalSave) {
    portalSave.addEventListener("click", async () => {
      ...
      const r = await api("/api/settings/portal", {
        method: "PUT",
        body: JSON.stringify({ logo_enabled, footer_text, theme }),
      });
      const j = await r.json().catch(() => ({}));
      setStatus("admin-portal-status", r.ok ? "Saved." : j.error || "Save failed");
      await loadPortalSettings();
