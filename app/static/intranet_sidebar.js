(function () {
  ...
  /** Sidebar is turbo-permanent; strip legacy sub-item styling so AI Chatbot aligns with peers. */
  function normalizeSidebarNavTabs() {
    const nav = document.querySelector(".nc-intranet-sidebar-nav");
    if (!nav) return;
    nav.querySelectorAll(".nc-intranet-tab.nc-intranet-tab--sub").forEach((tab) => {
