...
(function () {
  ...
  function setCallUi(mode) {
    ...
    if (callLead) {
      callLead.textContent = isJitsi
        ? "Voice call room — others in this chat should open the call too."
        : "Firmgate voice (WebRTC). Others in this chat must click the phone icon.";
    }
  ...
  function openJitsiCall() {
    ...
    setCallUi("jitsi");
    if (callStatus) callStatus.textContent = "Connected — allow microphone when prompted.";
    return true;
  }

  async function openCallViewer(opts = {}) {
    if (!callDialog || typeof callDialog.showModal !== "function") {
      window.alert("Voice calling is not supported in this browser.");
      return;
    }
    if (!activeRoomId) {
    ...
    setCallUi(voiceMode === "jitsi" ? "jitsi" : "webrtc");
    ...
    const useJitsi = voiceMode === "jitsi";
    if (useJitsi) {
      openJitsiCall();
      return;
    }
    ...
    if (!window.isSecureContext) {
      if (callStatus) {
        callStatus.textContent = "WebRTC needs HTTPS. Switching to compatibility mode…";
      }
      openJitsiCall();
      return;
    }
