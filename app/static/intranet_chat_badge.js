/** Team Chat unread count on intranet nav tab (red badge). */
(function () {
  const badge = document.getElementById("nc-intranet-tab-badge-team-chat");
  if (!badge) return;
  ...
  async function refresh() {
    try {
      const r = await fetch("/intranet/api/chat/unread", {
        method: "GET",
  ...
  window.ncChatNavBadge = { refresh, setCount };
