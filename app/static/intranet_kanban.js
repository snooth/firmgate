(function () {
  ...
  // Prevent double-initialisation (script is `data-turbo-eval="always"`, so it can
  // run again on the same DOM and stack duplicate listeners → duplicate cards).
  if (root.dataset.kanbanInit === "1") return;
  root.dataset.kanbanInit = "1";

  const canEdit = root.dataset.canEdit === "1";
  const canEditKanban = root.dataset.canEditKanban === "1";
  ...
  const createdByEl = document.getElementById("nc-kanban-created-by");
  ...
  // Server-recorded timestamps (notes, comments, activity) are stored as real
  // UTC. Display them through the portal's configured timezone + manual time
  // override so they match the time set in Administration.
  function formatServerTime(iso) {
    ...
    try {
      ...
      return new Date(base + APP_TIME_OFFSET_MS).toLocaleString("en-AU", {
  ...
  function noteMetaLine(note) {
    let line = formatServerTime(note.created_at);
    if (note.updated_at && note.updated_at !== note.created_at) {
      const editor = note.updated_by_name ? ` by ${note.updated_by_name}` : "";
      line += ` · edited${editor} ${formatServerTime(note.updated_at)}`;
    }
    return line;
  }
  ...
  function renderBoardColumns(data) {
    ...
    cols.forEach((col) => {
      ...
      const dot = col.color_token
        ...
        : "";
      const menuHtml =
        canEditKanban || canDelete
          ? `<div class="nc-kanban-col-head-actions">
              <div class="nc-crm2-col-meta" title="${esc(cardCountLabel((col.cards || []).length))}">${(col.cards || []).length}</div>
              <div class="nc-kanban-col-menu-wrap">
                <button type="button" class="nc-kanban-col-menu-btn" aria-label="Column options" aria-haspopup="menu" aria-expanded="false">⋯</button>
                <div class="nc-kanban-col-menu" role="menu" hidden>
                  ${canEditKanban ? '<button type="button" class="nc-kanban-col-menu-item" data-col-action="rename" role="menuitem">Rename column</button>' : ""}
                  ${canDelete ? '<button type="button" class="nc-kanban-col-menu-item nc-kanban-col-menu-item--danger" data-col-action="delete" role="menuitem">Delete column</button>' : ""}
                </div>
              </div>
            </div>`
          : `<div class="nc-crm2-col-meta" title="${esc(cardCountLabel((col.cards || []).length))}">${(col.cards || []).length}</div>`;
      head.innerHTML = `
        ...
        ${menuHtml}
      `;
      if (canEditKanban || canDelete) {
        const menuBtn = head.querySelector(".nc-kanban-col-menu-btn");
        const menuPop = head.querySelector(".nc-kanban-col-menu");
        menuBtn?.addEventListener("click", (ev) => {
          ev.stopPropagation();
          closeColumnMenus(menuPop);
          if (menuPop) {
            menuPop.hidden = !menuPop.hidden;
            menuBtn.setAttribute("aria-expanded", menuPop.hidden ? "false" : "true");
          }
        });
        head.querySelector('[data-col-action="rename"]')?.addEventListener("click", (ev) => {
          ev.stopPropagation();
          closeColumnMenus();
          void renameColumn(col);
        });
        head.querySelector('[data-col-action="delete"]')?.addEventListener("click", (ev) => {
          ev.stopPropagation();
          closeColumnMenus();
      ...
      <li class="nc-kanban-comment">
        <div class="nc-kanban-comment-head">
          ...
          <div>
            ...
            <div class="nc-kanban-comment-time">${esc(formatServerTime(c.created_at))}</div>
          </div>
        </div>
    ...
    activityList.innerHTML = rows
      ...
      <li class="nc-kanban-activity">
        ...
        <div>
          ...
          <div class="nc-kanban-activity-time">${esc(formatServerTime(a.created_at))}</div>
        </div>
      </li>`
      )
      .join("");
  ...
  function fillDetailForm(card) {
    ...
    if (createdByEl) {
      const creator = String(card.created_by_name || "").trim();
      if (creator) {
        createdByEl.textContent = `Created by: ${creator}`;
        ...
        .map(
          (card) => `<li class="nc-kanban-deleted-item">
            <div>
              ...
              <div class="nc-kanban-deleted-meta">${esc(card.column_title || "Column")} · deleted ${esc(formatServerTime(card.deleted_at))}${card.deleted_by_name ? ` by ${esc(card.deleted_by_name)}` : ""}</div>
            </div>
  ...
  async function saveDetails() {
    if (!canEdit) return;
    // Guard against double-submit (stacked listeners / fast double-click) that
    // would POST twice and create duplicate cards before dialogMode flips to "edit".
    ...
    try {
      ...
      activeCard = j.card;
      if (j.board) renderBoard(j.board);
      // If the user typed a note but didn't click "Post note", save it too.
      await postNote({ silent: true });
      closeCardDialog();
    } catch (err) {
  ...
  function columnApiUrl(columnId) {
    const base = String(apiColumns || "/intranet/api/kanban/columns").replace(/\/$/, "");
    return `${base}/${columnId}`;
  }

  function closeColumnMenus(exceptPop) {
    document.querySelectorAll(".nc-kanban-col-menu").forEach((pop) => {
      if (exceptPop && pop === exceptPop) return;
      pop.hidden = true;
      const wrap = pop.closest(".nc-kanban-col-menu-wrap");
      const btn = wrap ? wrap.querySelector(".nc-kanban-col-menu-btn") : null;
      if (btn) btn.setAttribute("aria-expanded", "false");
    });
  }

  async function renameColumn(col) {
    if (!canEditKanban || !col) return;
    const next = window.prompt("Column name", col.title || "");
    if (next == null) return;
    const title = String(next).trim();
    if (!title || title === col.title) return;
    try {
      const j = await api(columnApiUrl(col.id), {
        method: "PATCH",
        body: JSON.stringify(withBoardId({ title })),
      });
      renderBoard(j.board || {});
      setStatus("Column renamed.");
    } catch (err) {
      setStatus(err.message || "Could not rename column.");
    }
  }
  ...
  async function deleteColumn(col) {
    ...
    try {
      const j = await api(columnApiUrl(col.id), { method: "DELETE", body: JSON.stringify(withBoardId({})) });
      renderBoard(j.board || {});
      setStatus("Column deleted.");
    } catch (err) {
      setStatus(err.message || "Could not delete column.");
    }
  }
  ...
  document.addEventListener("click", (e) => {
    if (e.target && e.target.closest && e.target.closest(".nc-kanban-col-menu-wrap")) return;
    closeColumnMenus();
  });
  document.querySelectorAll(".nc-kanban-general-tab").forEach((btn) => {
  ...
  if (cardDialog) {
    ...
    // Card create/edit holds unsaved work — don't light-dismiss on backdrop
    // click or Esc. Closing requires the X button or saving changes.
