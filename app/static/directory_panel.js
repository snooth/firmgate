(function () {
  ...
  const docPiplExpiry = document.getElementById("nc-dir-doc-pipl-expiry");
  const docWcExpiry = document.getElementById("nc-dir-doc-wc-expiry");
  ...
  function cardHtmlForPerson(p) {
    ...
    const phoneRaw = p && p.phone ? p.phone : "";
    const phone = phoneRaw
      ? typeof window.ncSipPhoneLink === "function"
        ? window.ncSipPhoneLink(phoneRaw)
        : esc(phoneRaw)
      : "";
  ...
  function stripDirectoryOverlayFromUrl() {
    try {
      ...
      u.searchParams.delete("user_id");
      u.searchParams.delete("new_resource");
      u.searchParams.delete("workforce_edit");
  ...
  function companyPayloadFields() {
    return {
      ...
      document_expiry: {
        pi_pl_insurance: docPiplExpiry ? docPiplExpiry.value.trim() : "",
        workcover: docWcExpiry ? docWcExpiry.value.trim() : "",
      },
    };
  }

  function companyFieldsDirty(cur) {
    const pack = companyPayloadFields();
    if (!cur) return true;
    if (pack.abn !== (cur.abn || "")) return true;
    if (pack.acn !== (cur.acn || "")) return true;
    if (pack.company_rep !== (cur.company_rep || "")) return true;
    const docs = cur.documents || {};
    const piplExp = (docs.pi_pl_insurance && docs.pi_pl_insurance.expires_on) || "";
    const wcExp = (docs.workcover && docs.workcover.expires_on) || "";
    if (pack.document_expiry.pi_pl_insurance !== piplExp) return true;
    if (pack.document_expiry.workcover !== wcExp) return true;
    return false;
  }
  ...
  function syncCompanyDocLinks(cc) {
    ...
    function one(kind, statusEl, linkEl, expiryEl) {
      const d = docs[kind];
      if (statusEl) statusEl.textContent = d && d.original_name ? d.original_name : "";
      if (expiryEl) expiryEl.value = d && d.expires_on ? d.expires_on : "";
      if (linkEl) {
        if (d && d.url) {
          linkEl.href = d.url;
          linkEl.hidden = false;
    ...
    one("pi_pl_insurance", docPiplStatus, docPiplLink, docPiplExpiry);
    one("workcover", docWcStatus, docWcLink, docWcExpiry);
  }
  ...
  async function uploadCompanyDocument(companyId, kind, file) {
    ...
    if (!r.ok || !j.company) {
      ...
      if (!msg) {
        if (r.status === 413) {
          msg = "File too large for upload (server limit).";
        } else if (r.status === 403) {
          msg = "You don’t have permission to upload company documents.";
        } else if (r.status === 404) {
          msg = "Company not found — save the resource first, then upload.";
        } else {
          msg = `Upload failed (HTTP ${r.status}).`;
        }
      }
      throw new Error(msg);
    }
  ...
  function clearCompanyDocExpiryFields() {
    if (docPiplExpiry) docPiplExpiry.value = "";
    if (docWcExpiry) docWcExpiry.value = "";
  }
  ...
  async function ensurePersonLoaded(rawId) {
    const sid = canonicalPersonId(String(rawId || "").trim());
    if (!sid) return null;
  ...
  function updateCardFromPerson(person) {
    ...
    if (phoneEl) {
      ...
      if (ph && typeof window.ncSipPhoneLink === "function") {
        phoneEl.innerHTML = window.ncSipPhoneLink(ph);
      } else {
        ...
        if (sel === "__new__") {
          ...
          if (cur && companyFieldsDirty(cur)) {
            ...
            if (pj.company) {
              ...
              syncCompanyDocLinks(pj.company);
            }
          }
        }
      }
  ...
  function userPageHref(id) {
    return `/intranet/user/${encodeURIComponent(String(id || ""))}`;
  }

  function goToUserPage(id) {
    const sid = canonicalPersonId(String(id || "").trim());
    if (!sid) return;
    window.location.href = userPageHref(sid);
  }

  function handleDirectoryOverlayDeepLink() {
    if (!editDlg || !canEditProjects()) return;
    try {
      const sp = new URLSearchParams(window.location.search || "");
      if (sp.get("new_resource") === "1") {
      ...
      let userId = null;
      try {
        const intentEl = document.getElementById("directory-open-edit-user");
        if (intentEl) userId = JSON.parse(intentEl.textContent || "null");
      } catch (_) {
        userId = null;
      }
      if (userId == null || userId === "") {
        try {
          const stored = sessionStorage.getItem("nc.workforce.editUserId");
          if (stored) {
            userId = stored;
            sessionStorage.removeItem("nc.workforce.editUserId");
          }
        } catch (_) {}
      }
      if (userId == null || userId === "") {
        if (sp.get("workforce_edit") === "1") userId = sp.get("user_id");
      }
      if (!userId) return;

      const openForPerson = (person) => {
        if (!person) return;
        viewMode = "edit";
      ...
      const cached = personById(userId);
      if (cached) {
        openForPerson(cached);
        return;
      }
      ensurePersonLoaded(userId).then(openForPerson);
    } catch (_) {}
  }
  ...
  function wireCardGrid(g) {
    ...
    g.addEventListener("click", (e) => {
      ...
      e.preventDefault();
      goToUserPage(card.getAttribute("data-person-id"));
    });
  ...
  function docStatusEl(kind) {
    return kind === "pi_pl_insurance" ? docPiplStatus : kind === "workcover" ? docWcStatus : null;
  }
  function docInputEl(kind) {
    return kind === "pi_pl_insurance" ? docPiplInput : kind === "workcover" ? docWcInput : null;
  }
  function setDocStatus(kind, text) {
    const el = docStatusEl(kind);
  ...
  function queueOrUploadDoc(kind, file) {
    ...
    // Always remember the choice so Save can (re)confirm the upload even when
    // no company is selected yet, or the dropdown is changed afterwards.
    pendingCompanyDocs[kind] = file;
    if (cid && !Number.isNaN(cid)) {
      setDocStatus(kind, `Uploading ${label}…`);
      uploadCompanyDocument(cid, kind, file)
        .then(() => {
          // Persisted now — drop the pending copy so Save won't re-upload it.
          pendingCompanyDocs[kind] = null;
          const inp = docInputEl(kind);
          if (inp) inp.value = "";
        })
        .catch((err) => {
          setDocStatus(kind, String(err && err.message ? err.message : err) || "Upload failed");
        });
      return;
    }
    setDocStatus(kind, `Ready: ${label} (uploads after save)`);
  }

  function refreshPendingDocLabels() {
    ["pi_pl_insurance", "workcover"].forEach((kind) => {
      const f = pendingCompanyDocs[kind];
      if (f) setDocStatus(kind, `Ready: ${f.name || "file"} (uploads after save)`);
    });
  }
  ...
  document.querySelectorAll(".nc-dir-doc-dropzone[data-doc-kind]").forEach((zone) => {
    const kind = zone.getAttribute("data-doc-kind");
    if (!kind) return;
    const stop = (e) => {
      e.preventDefault();
      e.stopPropagation();
  ...
  window.addEventListener("popstate", (e) => {
    ...
    if (st) {
      ...
      if (id) {
        ...
        ensurePersonLoaded(id).then((p) => {
          if (p) {
            openEditDialog(p, { pushHistory: false });
            setFormMode(viewMode);
    ...
    if (urlHasOverlay && editDlg && !editDlg.hidden) return;
  ...
  // Deep-link: /intranet/directory?user_id=<id>&workforce_edit=1 or ?new_resource=1
  window.requestAnimationFrame(() => handleDirectoryOverlayDeepLink());
