(function () {
  function computeViewerTopPx() {
    try {
      ...
      const chrome =
        document.querySelector(".nc-intranet-topbar") ||
        document.querySelector(".nc-intranet-top") ||
        document.querySelector(".nc-intranet-stage") ||
  ...
  function syncViewerOffsets() {
    ...
    try {
      const top = document.querySelector(".nc-intranet-topbar") || document.querySelector(".nc-intranet-top");
      if (top) document.documentElement.style.setProperty("--nc-intranet-top-h", `${Math.round(top.offsetHeight || 0)}px`);
      /* Sidebar width comes from CSS (--nc-intranet-sidebar-w on html). Do not measure here:
         inline width overrides collapsed state and blocks the main column from expanding. */
      document.documentElement.style.removeProperty("--nc-intranet-sidebar-w");
      document.documentElement.style.setProperty("--nc-intranet-tabs-h", "0px");
    } catch (_) {}
  }
