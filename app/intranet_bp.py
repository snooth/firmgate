...
def _nav(active: str) -> dict:
    ...
    about_sub_items: list[tuple[str, str, str]] = []
    if any(it[0] == "about" for it in filtered):
        try:
            from app.sip_client import sip_configured

            if sip_configured():
                about_sub_items = [("about_phone", "Phone", "intranet.phone_page")]
        except Exception:
            about_sub_items = []
    ...
    return {
        ...
        "about_sub_items": about_sub_items,
    }
...
def _chat_room_visible_to_user(room_id: int) -> bool:
    return _chat_user_can_access_room(room_id, int(current_user.id))


def _chat_user_can_access_room(room_id: int, user_id: int) -> bool:
    """Room member, or portal administrator (``admin.all``)."""
    row = (
        db.session.query(ChatRoomMember.id)
        .filter(ChatRoomMember.room_id == room_id, ChatRoomMember.user_id == int(user_id))
        .first()
    )
    if row is not None:
        return True
    ...
    return bool(u and u.is_active and rbac.user_has_permission(u, rbac.PERMISSION_ADMIN))
...
@bp.route("/api/team-chat/signaling-config", methods=["GET"])
@login_required
def api_team_chat_signaling_config():
    from app.intranet_signaling import team_chat_signaling_for_client

    return jsonify(team_chat_signaling_for_client())
...
def api_chat_call_signals_post(room_id: int):
    ...
    if kind in ("offer", "answer", "ice"):
        ...
        if not _chat_user_can_access_room(room_id, to_uid):
    ...
    if kind == "join":
        from app.intranet_signaling import get_intranet_signaling_settings
        ...
        if cfg.get("enabled"):
            ...
            if me not in active and len(active) >= max_p:
                return jsonify({"error": f"Call is full (max {max_p} participants)."}), 409
...
def _contractor_company_public_dict(c: ContractorCompany) -> dict:
    ...
    for key in ("pi_pl_insurance", "workcover"):
        ...
        out: dict[str, str] = {}
        if entry.get("stored"):
            out["original_name"] = str(entry.get("original_name") or entry.get("name") or "document")
            out["url"] = f"/intranet/media/contractor-company/{c.id}/{entry['stored']}"
        exp = str(entry.get("expires_on") or "").strip()
        if exp:
            out["expires_on"] = exp
        if out:
            docs_out[key] = out
...
def _parse_contractor_doc_expiry(value: object) -> str | None:
...
def apply_contractor_document_expiry(c: ContractorCompany, expiry_raw: object) -> None:
    """Merge expiry dates into company documents JSON."""
    if not isinstance(expiry_raw, dict):
        return
    docs = dict(c.documents) if isinstance(c.documents, dict) else {}
    for kind in ("pi_pl_insurance", "workcover"):
        if kind not in expiry_raw:
            continue
        parsed = _parse_contractor_doc_expiry(expiry_raw.get(kind))
        entry = dict(docs.get(kind) or {})
        if parsed:
            entry["expires_on"] = parsed
        elif "expires_on" in entry:
...
def team_chat_page():
    ...
    q = (request.args.get("q") or "").strip()
    signaling = team_chat_signaling_for_client()
    webrtc_stun = ""
    if signaling.get("ice_servers"):
        first = signaling["ice_servers"][0]
        if isinstance(first, dict):
            webrtc_stun = str(first.get("urls") or "").strip()
    voice_mode = "webrtc" if signaling.get("enabled") else str(current_app.config.get("VOICE_CALL_MODE") or "webrtc").strip().lower()
...
@bp.route("/about/phone", methods=["GET"])
@login_required
def phone_page():
    from app.sip_client import sip_configured
    ...
    return render_template(
        "intranet_phone.html",
        nav=_nav("about_phone"),
        sip_enabled=True,
        sip_configured=True,
    )


@bp.route("/api/sip/status", methods=["GET"])
@login_required
def api_sip_status():
    from app.sip_client import user_sip_status

    return jsonify({"ok": True, **user_sip_status(current_user.id)})


@bp.route("/api/sip/register-config", methods=["GET"])
@login_required
def api_sip_register_config():
    from app.sip_client import current_user_display_name, sip_configured, user_sip_registration_config

    if not sip_configured():
        return jsonify({"ok": False, "error": "SIP phone is not configured."}), 503
    cfg = user_sip_registration_config(
        current_user.id,
        default_display_name=current_user_display_name(),
        secure=request.is_secure,
    )
    if not cfg:
        return jsonify({"ok": False, "error": "Set your extension and SIP password on this page first."}), 400
    return jsonify({"ok": True, "config": cfg})


@bp.route("/api/sip/credentials", methods=["PUT"])
@login_required
def api_sip_credentials_put():
    from app.sip_client import save_user_sip_credentials, sip_configured

    if not sip_configured():
        return jsonify({"ok": False, "error": "SIP phone is not configured."}), 503
    payload = request.get_json(force=True, silent=True) or {}
    result = save_user_sip_credentials(current_user.id, payload)
    if isinstance(result, tuple):
        body, code = result
        return jsonify({"ok": False, **body}), code
    return jsonify({"ok": True, **result})


@bp.route("/api/sip/stay-registered", methods=["PUT"])
@login_required
def api_sip_stay_registered_put():
    from app.sip_client import set_user_stay_registered, sip_configured, user_sip_status
    ...
    set_user_stay_registered(current_user.id, bool(payload.get("enabled")))
    return jsonify({"ok": True, **user_sip_status(current_user.id)})
