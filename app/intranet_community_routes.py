"""Community intranet routes (wiki, documents, workforce APIs).

Registered from app.intranet_bp.
"""
...
from app.intranet_bp import (
    ...
    apply_contractor_document_expiry,
...
def api_contractor_companies():
    ...
    apply_contractor_document_expiry(c, payload.get("document_expiry"))
    db.session.add(c)
    db.session.commit()
...
def api_contractor_company_document_upload(company_id: int, kind: str):
    ...
    if not upload_root:
        current_app.logger.error("contractor doc upload: UPLOAD_ROOT not configured")
        return jsonify({"error": "Upload storage is not configured."}), 500
    out_dir = Path(str(upload_root)) / "contractor_companies" / str(company_id)
    try:
        out_dir.mkdir(parents=True, exist_ok=True)
        out_path = out_dir / stored
        f.save(out_path)
    except OSError as exc:
        current_app.logger.exception("contractor doc upload: could not save file")
        return jsonify({"error": f"Could not save file: {exc}"}), 500
    docs = dict(c.documents) if isinstance(c.documents, dict) else {}
    prev = dict(docs.get(kind) or {})
    prev["original_name"] = orig
    prev["stored"] = stored
    docs[kind] = prev
    c.documents = docs
    try:
        ...
        db.session.rollback()
        current_app.logger.exception("contractor doc upload: commit failed")
        return jsonify({"error": "Could not save document record."}), 500
    return jsonify({"company": _contractor_company_public_dict(c)}), 201
...
def api_wiki_page_patch(slug: str):
    ...
    try:
        from app.module_email_notifications import notify_wiki_page_updated

        notify_wiki_page_updated(r, editor=current_user)
    except Exception:
        pass
...
def _kanban_can_edit_kanban(board: KanbanBoard | None = None) -> bool:
    from app.kanban_service import kanban_can_edit_kanban

    target = board or _kanban_board()
    return kanban_can_edit_kanban(current_user, target)
...
def _kanban_notify_safe(fn, *args, **kwargs) -> None:
    ...
    if not isinstance(result, dict):
        return
    event = result.get("event") or getattr(fn, "__name__", "kanban")
    ...
    if sent:
        current_app.logger.info("KanBan notify %s sent %s email(s)", event, sent)
    if failed:
        msgs = result.get("messages") or []
        current_app.logger.warning(
            "KanBan notify %s failed for %s recipient(s)%s",
            event,
            failed,
            f": {'; '.join(str(m) for m in msgs[:3])}" if msgs else "",
        )
...
def kanban_board_page(board_id: int):
    ...
    return render_template(
        ...
        kanban_can_edit_kanban=_kanban_can_edit_kanban(board),
...
def api_kanban_column_create():
    ...
    if not _kanban_can_edit_kanban(board):
        return jsonify({"error": "forbidden"}), 403
...
def api_kanban_card_mutate(card_id: int):
    ...
    if request.method == "GET":
        ...
        return jsonify({"card": serialize_card_detail(card, viewer=current_user)})
    ...
    prev_assignee_ids = card_assignee_ids(card)
    changes: dict[str, object] = {}
    if "title" in payload:
        title = str(payload.get("title") or "").strip()[:255]
        if not title:
    ...
    if "assignee_ids" in changes:
        added_ids = [uid for uid in (changes.get("assignee_ids") or []) if int(uid) not in set(prev_assignee_ids)]
        if added_ids:
            _kanban_notify_safe(notify_card_assigned, card, current_user, previous_ids=prev_assignee_ids)
...
def api_kanban_card_comment_create(card_id: int):
    ...
    return jsonify({"ok": True, "card": serialize_card_detail(card, viewer=current_user)})


@bp.route("/api/kanban/cards/<int:card_id>/notes", methods=["POST"])
@login_required
def api_kanban_card_note_create(card_id: int):
    card = _kanban_card_or_404(card_id)
    board = _kanban_board_for_card(card)
    if not _kanban_can_read_board(board):
        return jsonify({"error": "forbidden"}), 403
...
@bp.route("/api/kanban/cards/<int:card_id>/notes/<int:note_id>", methods=["PATCH", "DELETE"])
@login_required
def api_kanban_card_note_mutate(card_id: int, note_id: int):
    card = _kanban_card_or_404(card_id)
    board = _kanban_board_for_card(card)
    if not _kanban_can_read_board(board):
        return jsonify({"error": "forbidden"}), 403
    ...
    row = db.session.get(KanbanCardNote, note_id)
    if not row or int(row.card_id) != int(card.id):
        return jsonify({"error": "not found"}), 404
    ...
    if "muted" in payload:
        ...
        if muted is True:
            row.muted_at = utcnow()
            row.muted_by_id = int(current_user.id)
            log_kanban_activity(
                card,
                user_id=int(current_user.id),
                action="note_muted",
                details={"preview": (row.body or "")[:120]},
            )
            changed = True
        elif muted is False:
            row.muted_at = None
            row.muted_by_id = None
            log_kanban_activity(
                card,
                user_id=int(current_user.id),
                action="note_unmuted",
            ...
            return jsonify({"error": "invalid muted value"}), 400
    if "body" in payload:
        body = str(payload.get("body") or "").strip()[:4000]
        if not body:
            return jsonify({"error": "body required"}), 400
        ...
        row.updated_by_id = int(current_user.id)
        log_kanban_activity(
            card,
            user_id=int(current_user.id),
            action="note_edited",
