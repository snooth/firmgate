"""Fixed module allowlist and default navigation policy."""
...
# Modules included in the open-source release.
...
def community_module_available(module_key: str) -> bool:
    ...
    if not is_community_edition():
        return True
    return key in COMMUNITY_EDITION_MODULES


def default_community_module_rows() -> dict[str, dict[str, Any]]:
    """Default Administration → Modules policy."""
...
def apply_community_module_policy(mods: dict[str, Any]) -> dict[str, dict[str, Any]]:
    """Keep community modules as stored, seeding defaults for unset keys."""
    if not is_community_edition():
        return mods
...
def ensure_community_module_defaults() -> None:
    """On startup, seed module policy when unset."""
    if not is_community_edition():
        return
