...
def send_email(
    *,
    ...
    for att in attachments or []:
        ...
        msg.add_attachment(data, maintype=mime.split("/", 1)[0], subtype=mime.split("/", 1)[-1], filename=fname)

    all_recipients = recipients + [a for a in cc_list if a.lower() not in {r.lower() for r in recipients}]
    try:
        if use_ssl:
    ...
    if cc_list:
        sent_to = f"{sent_to} (Cc: {', '.join(cc_list)})"
    return True, f"Message sent to {sent_to}."
