...
RELEASES_DIR="$ROOT/PRIVATE/RELEASES"
mkdir -p "$RELEASES_DIR"
cp -a "$ZIP_PATH" "$RELEASES_DIR/$ZIP_NAME"
if [ ! -f "$RELEASES_DIR/README.md" ]; then
  cat >"$RELEASES_DIR/README.md" <<'EOF'
# Release packages

ZIP files here are produced by `scripts/build_release_package.sh` from the workspace root.

They are intended for **Administration → Software version → Upgrade from package** (when package upgrades are enabled). Each build adds a new timestamped archive; older files are kept until you remove them.

This folder lives under **PRIVATE/** (not published to public GitHub).
EOF
fi

rm -rf "$STAGING"
echo "Built: $ZIP_PATH"
echo "Private release copy: $RELEASES_DIR/$ZIP_NAME"
