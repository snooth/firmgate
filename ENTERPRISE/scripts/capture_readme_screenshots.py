#!/usr/bin/env python3
"""Capture README gallery screenshots from a running Firmgate instance (Community Edition)."""
...
# Community Edition README gallery (public GitHub).
COMMUNITY_SHOTS: list[tuple[str, str, str | None]] = [
    ...
    ("security-officer.png", "/intranet/security-officer", None),
    ...
    ("enterprise-features.png", "/intranet/admin", "premium_features"),
]

# Full enterprise build only (COMMUNITY_EDITION=0 or demo on enterprise install).
ENTERPRISE_SHOTS: list[tuple[str, str, str | None]] = [
...
def _prepare_view(page, path: str, extra: str | None) -> None:
    ...
    if path.endswith("/team-chat"):
        # Open first room so the conversation pane (current UI) is visible.
...
def main() -> None:
    with sync_playwright() as p:
        ...
        _login(page)
        _capture_set(page, COMMUNITY_SHOTS, OUT)
        _capture_set(page, ENTERPRISE_SHOTS, ENTERPRISE_OUT)
        browser.close()
