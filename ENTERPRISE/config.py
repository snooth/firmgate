...
class Config:
    ...
    # When false, admin package upload upgrade API returns 503.
    ENABLE_SOFTWARE_PACKAGE_UPGRADE = os.environ.get("ENABLE_SOFTWARE_PACKAGE_UPGRADE", "1").lower() in (
        "1",
        "true",
        "yes",
    )
    ...
    # Community Edition (1): fixed module allowlist; premium features require a license key.
    COMMUNITY_EDITION = os.environ.get("COMMUNITY_EDITION", "1").lower() in ("1", "true", "yes")
    # Ed25519 public key (base64, 32 bytes) for FG2 license verification; optional if
    # app/enterprise_license_public.b64 is shipped with the build.
    FIRMGATE_LICENSE_PUBLIC_KEY = os.environ.get("FIRMGATE_LICENSE_PUBLIC_KEY") or ""
