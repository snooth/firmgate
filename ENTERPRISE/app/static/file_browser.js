(function () {
  ...
  const filesNavSelect = document.getElementById("nc-files-nav-select");

  function leftNavItemLabel(btn) {
    const clone = btn.cloneNode(true);
    clone.querySelectorAll(".nc-leftnav-ic, .nc-leftnav-chevron").forEach((el) => el.remove());
    return (clone.textContent || "").replace(/\s+/g, " ").trim();
  }

  function rebuildFilesMobileNav() {
    if (!filesNavSelect) return;
    const items = [...document.querySelectorAll(".nc-leftnav-item[data-nav]:not([hidden])")];
    filesNavSelect.replaceChildren();
    for (const btn of items) {
      const opt = document.createElement("option");
      opt.value = btn.dataset.nav || "";
      opt.textContent = leftNavItemLabel(btn) || btn.dataset.nav || "";
      filesNavSelect.appendChild(opt);
    }
    syncFilesMobileNav();
  }

  function syncFilesMobileNav() {
    if (!filesNavSelect || !filesNavSelect.options.length) return;
    if ([...filesNavSelect.options].some((o) => o.value === leftNavMode)) {
      filesNavSelect.value = leftNavMode;
    }
  }
  ...
  document.addEventListener(
    "click",
    (e) => {
      const a = e.target && e.target.closest && e.target.closest(".nc-intranet-sidebar-nav a.nc-intranet-tab, .nc-intranet-tabs a.nc-intranet-tab");
      if (!a) return;
      const anyOpen =
  ...
  function wireDrag(el, it, data) {
    ...
    el.addEventListener("dragover", (e) => {
      if (hasFilePayload(e.dataTransfer)) {
        const uploadTarget = it.is_folder ? it.id : effectiveParentId();
  ...
  filesNavSelect?.addEventListener("change", () => {
    const mode = filesNavSelect.value;
    const btn = document.querySelector(`.nc-leftnav-item[data-nav="${mode}"]`);
    if (btn) btn.click();
  });
