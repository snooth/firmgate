"""Persisted conversation threads for AI Policy Assistant."""
...
from app.models import AiPolicyConversation, User, utcnow
...
def _ensure_conversations_schema() -> None:
    try:
        ...
        if not insp.has_table("ai_policy_conversations"):
            db.create_all()
    except Exception:
        try:
            db.create_all()
...
def list_conversations(user: User) -> list[dict[str, Any]]:
    ...
    rows = (
        db.session.query(AiPolicyConversation)
        .filter_by(user_id=user.id)
        .order_by(AiPolicyConversation.updated_at.desc())
...
def get_conversation(user: User, conversation_id: int) -> dict[str, Any] | None:
    _ensure_conversations_schema()
    row = db.session.get(AiPolicyConversation, int(conversation_id))
    ...
    welcome = [
        {
            "role": "assistant",
            "content": (
                "Upload policies, SOPs, work instructions, and security manuals, then ask questions. "
                "I answer from your uploaded material and cite sources."
            ),
            "welcome": True,
        }
    ]
    row = AiPolicyConversation(
        user_id=user.id,
...
def _trim_old_conversations(user_id: int) -> None:
    rows = (
        db.session.query(AiPolicyConversation.id)
    ...
    for (cid,) in rows[_MAX_CONVERSATIONS_PER_USER:]:
        db.session.query(AiPolicyConversation).filter_by(id=cid).delete()
    db.session.commit()
