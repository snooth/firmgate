...
from flask import Blueprint, abort, flash, jsonify, redirect, render_template, request, current_app, url_for, send_file
...
def _intranet_rel_path() -> str:
    """Request path relative to the intranet blueprint prefix (always starts with '/')."""
    p = str(getattr(request, "path", "") or "")
    if p.startswith("/intranet"):
        rel = p[len("/intranet") :]
...
def _security_officer_module_allowed(user: User) -> bool:
    """Security Officer nav/page access (Administration → Modules policy)."""
...
def _nav(active: str) -> dict:
    items = [
        ...
        ("timesheets", "Timesheets", "intranet.timesheets_page"),
        ...
        ("ai_document_search", "AI Document Search", "intranet.ai_document_search_page"),
        ("ai_chatbot", "AI Chatbot", "intranet.ai_chatbot_page"),
        ("ai_policy_assistant", "AI Docs and Policy", "intranet.ai_policy_assistant_page"),
        ("ai_cv_builder", "AI CV Builder", "intranet.ai_cv_builder_page"),
        ("ai_tender_assistant", "AI Tender Assistant", "intranet.ai_tender_assistant_page"),
        ...
        ("resource_calculator", "Casual Calculator", "intranet.resource_calculator_page"),
    ...
    try:
        ...
        from app.premium_license import AI_ENTERPRISE_NAV_MODULES, ai_nav_module_licensed

        def _nav_item_allowed(key: str) -> bool:
            ...
            if rule.get("enabled") is False:
                if key == "admin" and (is_admin or can_access_users_admin):
                    ...
                    return False
            if key in AI_ENTERPRISE_NAV_MODULES:
                return ai_nav_module_licensed(key)
            if is_admin or (key == "admin" and can_access_users_admin):
                return True
            if key == "security_officer":
                return bool(
                    current_user.is_authenticated and _security_officer_module_allowed(current_user)
                )
            if not bool(rule.get("restricted")):
                return True
            ids = rule.get("allowed_user_ids")
        ...
        allowed: set[str] = set()
        timesheets_collection_allowed = False
        for key, _label, _endpoint in items:
            if _nav_item_allowed(key):
                allowed.add(key)
        timesheets_collection_allowed = _nav_item_allowed("timesheets_collection")
    except Exception:
    ...
    try:
        from app.premium_license import AI_ENTERPRISE_NAV_MODULES

        enterprise_ai = frozenset(k for k in AI_ENTERPRISE_NAV_MODULES if k in allowed)
    except Exception:
        enterprise_ai = frozenset()
    try:
        from app.community_edition import ENTERPRISE_ONLY_MODULES

        enterprise_mods = frozenset(k for k in ENTERPRISE_ONLY_MODULES if k in allowed)
    except Exception:
        enterprise_mods = frozenset()

    enterprise_keys = enterprise_ai | enterprise_mods
    standard_items: list[tuple[str, str, str]] = []
    enterprise_items: list[tuple[str, str, str]] = []
    admin_items: list[tuple[str, str, str]] = []
    for it in filtered:
        key = it[0]
        if key in enterprise_keys:
            enterprise_items.append(it)
        elif key == "admin":
            admin_items.append(it)
        else:
            standard_items.append(it)

    has_timesheets = any(it[0] == "timesheets" for it in filtered)
    timesheets_sub_items: list[tuple[str, str, str]] = []
    if has_timesheets and timesheets_collection_allowed:
        timesheets_sub_items = [
...
def security_officer_page():
    ...
    abort_if_module_locked("security_officer")
...
def directory_page():
    ...
    new_resource = (request.args.get("new_resource") or "").strip() == "1"
    workforce_edit = (request.args.get("workforce_edit") or "").strip() == "1"
    workforce_open_edit_user_id = None
    if workforce_edit and uid_raw.isdigit() and _workforce_can_create():
        workforce_open_edit_user_id = int(uid_raw)
    if uid_raw.isdigit() and not new_resource and not workforce_edit:
        return redirect(url_for("intranet.user_page", user_id=int(uid_raw)))
    q = (request.args.get("q") or "").strip()
    qq = q.lower()
    ...
    return render_template(
        ...
        workforce_open_edit_user_id=workforce_open_edit_user_id,
    )
...
def _timesheets_nav(active_sub: str) -> dict:
    active = "timesheets_collection" if active_sub == "collection" else "timesheets"
    return _nav(active)


def _format_timesheet_month_label(month: str) -> str:
    try:
...
def _format_timesheet_uploaded_at(iso: str) -> str:
...
def _timesheet_collection_can_upload() -> bool:
    return bool(
        current_user.is_authenticated
        and (
            rbac.user_has_permission(current_user, rbac.PERMISSION_ADMIN)
            or rbac.user_can_access_users_admin(current_user)
        )
    )


def _timesheet_collection_roster() -> list[User]:
    base_raw = (
        db.session.query(User)
        .filter(User.is_active.is_(True))
        .order_by(User.full_name.asc(), User.username.asc())
    ...
    users, _ = _dedupe_users_by_email(base_raw)
    return users


def _timesheet_collection_row_dict(user: User, month: str, signed: dict | None) -> dict[str, Any]:
    employee_name = (user.full_name or user.username or f"User {user.id}").strip()
    if signed:
        return {
            "user_id": user.id,
            "employee_name": employee_name,
            "uploaded": True,
            "original_name": str(signed.get("original_name") or ""),
            "uploaded_label": _format_timesheet_uploaded_at(signed.get("uploaded_at") or ""),
            "download_url": url_for(
                "intranet.api_timesheets_collection_download",
                user_id=user.id,
                month=month,
            ),
        }
...
def _timesheet_collection_rows(month: str) -> list[dict[str, Any]]:
    from app.timesheet_signed import list_all_signed_timesheets, normalize_month

    month = normalize_month(month)
    signed_map = {
        r["user_id"]: r
        for r in list_all_signed_timesheets()
        if r["month"] == month
    }
    out = [
        _timesheet_collection_row_dict(u, month, signed_map.get(u.id))
        for u in _timesheet_collection_roster()
    ]
    out.sort(key=lambda r: r["employee_name"].lower())
    return out


def _timesheet_collection_row_api(row: dict[str, Any]) -> dict[str, Any]:
    return {
        "user_id": row["user_id"],
        "employee_name": row["employee_name"],
        "uploaded": row["uploaded"],
        "original_name": row["original_name"],
        "uploaded_label": row["uploaded_label"],
        "download_url": row["download_url"],
    }


def _timesheet_collection_sections(month: str) -> list[dict[str, Any]]:
    from app.timesheet_collection_groups import group_collection_rows

    rows = _timesheet_collection_rows(month)
    return group_collection_rows(rows)


def _timesheet_collection_month() -> str:
    from app.timesheet_signed import normalize_month

    month = (request.args.get("month") or "").strip()
    default = datetime.utcnow().strftime("%Y-%m")
    try:
        return normalize_month(month) if month else normalize_month(default)
    except ValueError:
        return normalize_month(default)


def _timesheet_collection_export_user_ids() -> list[int] | None:
    """Explicit user filter from query params, or None to include all roster rows."""
    ids: list[int] = []
    for raw in request.args.getlist("user_id"):
        try:
            n = int(raw)
        except (TypeError, ValueError):
            continue
        if n not in ids:
            ids.append(n)
    extra = (request.args.get("user_ids") or "").strip()
    if extra:
...
def _timesheet_collection_rows_for_export(month: str) -> list[dict[str, Any]]:
    rows = _timesheet_collection_rows(month)
    user_ids = _timesheet_collection_export_user_ids()
    if not user_ids:
        return rows
    wanted = set(user_ids)
    return [r for r in rows if r.get("user_id") in wanted]
...
def timesheets_page():
    from app.community_edition import abort_if_module_locked
    from app.timesheet_holidays import DEFAULT_TIMESHEET_STATE, TIMESHEET_AU_SUBDIVS, holidays_for_year
    from app.timesheet_user_prefs import get_user_prefs

    abort_if_module_locked("timesheets")
    q = (request.args.get("q") or "").strip()
    user_name = (current_user.full_name or current_user.username or "").strip()
    job_title = _directory_job_title(current_user) or "Lead Engineer"
    year = datetime.utcnow().year
    user_state = get_user_prefs(current_user.id).get("state") or DEFAULT_TIMESHEET_STATE
    name_for_declaration = user_name or "[your name]"
    declaration_default = (
        f"Declaration: I {name_for_declaration} certify that the hours shown accurately represent my total "
        "hours worked for Client Program — Delivery."
    )
    return render_template(
        "intranet_timesheets.html",
        nav=_timesheets_nav("mine"),
        q=q,
        ts_user_name=user_name,
        ts_job_title=job_title,
        ts_declaration_default=declaration_default,
        ts_states=TIMESHEET_AU_SUBDIVS,
        ts_default_state=user_state,
        ts_holidays=holidays_for_year(year, state=user_state),
    )


@bp.route("/timesheets/collection", methods=["GET"])
@login_required
def timesheets_collection_page():
    from app.community_edition import abort_if_module_locked

    abort_if_module_locked("timesheets_collection")
    month = _timesheet_collection_month()
    sections = _timesheet_collection_sections(month)
    return render_template(
        "intranet_timesheets_collection.html",
        nav=_timesheets_nav("collection"),
        ts_collection_month=month,
        ts_collection_month_label=_format_timesheet_month_label(month),
        ts_collection_can_upload=_timesheet_collection_can_upload(),
        collection_sections=sections,
    )


@bp.route("/api/timesheets/collection", methods=["GET"])
@login_required
def api_timesheets_collection():
    from app.community_edition import abort_if_module_locked
    from app.timesheet_signed import normalize_month
    ...
    month = (request.args.get("month") or "").strip()
    try:
        month = normalize_month(month) if month else normalize_month(datetime.utcnow().strftime("%Y-%m"))
    ...
    return jsonify(
        {
            "ok": True,
            "month": month,
            "month_label": _format_timesheet_month_label(month),
            "rows": [_timesheet_collection_row_api(r) for r in rows],
            "sections": [
                {
                    "id": s.get("id") or "",
                    "name": s.get("name") or "",
                    "show_header": bool(s.get("show_header")),
                    "rows": [_timesheet_collection_row_api(r) for r in s.get("rows") or []],
                }
                for s in sections
            ],
            "can_upload": _timesheet_collection_can_upload(),
        }
    )


@bp.route("/api/timesheets/collection/upload", methods=["POST"])
@login_required
def api_timesheets_collection_upload():
    from app.community_edition import abort_if_module_locked
    from app.timesheet_signed import normalize_month, save_signed_timesheet

    abort_if_module_locked("timesheets_collection")
    if not _timesheet_collection_can_upload():
        return jsonify({"error": "forbidden"}), 403
    ...
    if ext != "pdf" and mt not in ("application/pdf", "application/x-pdf"):
        return jsonify({"error": "signed timesheet must be a PDF"}), 400
    ...
    try:
        month = normalize_month(month)
        info = save_signed_timesheet(user_id, month, f, f.filename)
    except ValueError as exc:
        return jsonify({"error": str(exc)}), 400
    except Exception as exc:
        return jsonify({"error": str(exc) or "Could not save signed timesheet."}), 400

    row = _timesheet_collection_row_dict(user, month, info)
    return jsonify(
        {
            "ok": True,
            "month": month,
...
@bp.route("/api/timesheets/collection/remove", methods=["POST"])
@login_required
def api_timesheets_collection_remove():
    from app.community_edition import abort_if_module_locked
    from app.timesheet_signed import delete_signed_timesheet, normalize_month
    ...
    try:
        ...
        return jsonify({"error": str(exc)}), 400
    if not delete_signed_timesheet(user_id, month):
        return jsonify({"error": "No signed timesheet for this month."}), 404

    row = _timesheet_collection_row_dict(user, month, None)
    return jsonify(
        {
            "ok": True,
            "month": month,
...
@bp.route("/api/timesheets/collection/download", methods=["GET"])
@login_required
def api_timesheets_collection_download():
    from app.community_edition import abort_if_module_locked
    from app.timesheet_signed import get_signed_timesheet, normalize_month, signed_file_path
    ...
    try:
        ...
        return jsonify({"error": str(exc)}), 400
    path = signed_file_path(user_id, month)
    if not path:
        return jsonify({"error": "not found"}), 404
    info = get_signed_timesheet(user_id, month) or {}
    name = str(info.get("original_name") or path.name)
    return send_file(path, mimetype="application/pdf", as_attachment=False, download_name=name)


@bp.route("/api/timesheets/collection/download-all", methods=["GET"])
@login_required
def api_timesheets_collection_download_all():
    from app.community_edition import abort_if_module_locked
    from app.timesheet_collection_export import build_signed_timesheets_zip
    from app.timesheet_signed import normalize_month
    ...
    try:
        ...
        return jsonify({"error": str(exc)}), 400
    rows = _timesheet_collection_rows_for_export(month)
    buf = build_signed_timesheets_zip(month, rows)
    if not buf:
        msg = "No signed timesheets for the selected employees."
        if _timesheet_collection_export_user_ids():
            return jsonify({"error": msg}), 404
        return jsonify({"error": "No signed timesheets for this month."}), 404
    download_name = f"timesheet-collection-{month}.zip"
    return send_file(
        buf,
...
@bp.route("/api/timesheets/collection/export-docx", methods=["GET"])
@login_required
def api_timesheets_collection_export_docx():
    from app.community_edition import abort_if_module_locked
    from app.timesheet_collection_export import build_monthly_timesheets_docx, monthly_timesheets_docx_filename
    from app.timesheet_signed import normalize_month
    ...
    rows = _timesheet_collection_rows_for_export(month)
    try:
        buf = build_monthly_timesheets_docx(month, rows)
    ...
    if not buf:
        ...
        if _timesheet_collection_export_user_ids():
            return jsonify({"error": msg}), 404
        return jsonify({"error": "No signed timesheets for this month."}), 404
    download_name = monthly_timesheets_docx_filename(month)
    return send_file(
        buf,
...
@bp.route("/api/timesheets/collection/export-pdf", methods=["GET"])
@login_required
def api_timesheets_collection_export_pdf():
    from app.community_edition import abort_if_module_locked
    from app.timesheet_collection_export import build_monthly_timesheets_pdf, monthly_timesheets_pdf_filename
    from app.timesheet_signed import normalize_month
    ...
    rows = _timesheet_collection_rows_for_export(month)
    try:
        buf = build_monthly_timesheets_pdf(month, rows)
    except RuntimeError as exc:
        return jsonify({"error": str(exc)}), 500
    if not buf:
        msg = "No signed timesheets for the selected employees."
        if _timesheet_collection_export_user_ids():
            return jsonify({"error": msg}), 404
        return jsonify({"error": "No signed timesheets for this month."}), 404
    download_name = monthly_timesheets_pdf_filename(month)
    return send_file(
        buf,
        mimetype="application/pdf",
        as_attachment=True,
...
@bp.route("/api/timesheets/holidays", methods=["GET"])
@login_required
def api_timesheets_holidays():
    from app.community_edition import abort_if_module_locked
    from app.timesheet_holidays import holidays_for_year, normalize_state
    ...
    return jsonify({"ok": True, "year": year, "state": state or "", "holidays": holidays_for_year(year, state=state)})


@bp.route("/api/timesheets/prefs", methods=["GET"])
@login_required
def api_timesheets_prefs_get():
    from app.community_edition import abort_if_module_locked
    from app.timesheet_user_prefs import get_user_prefs

    abort_if_module_locked("timesheets")
    return jsonify({"ok": True, **get_user_prefs(current_user.id)})


@bp.route("/api/timesheets/prefs", methods=["PUT"])
@login_required
def api_timesheets_prefs_put():
    from app.community_edition import abort_if_module_locked
    from app.timesheet_user_prefs import save_user_prefs
    ...
    try:
        out = save_user_prefs(current_user.id, state)
    except ValueError as exc:
        return jsonify({"error": str(exc)}), 400
    return jsonify({"ok": True, **out})


@bp.route("/api/timesheets/month", methods=["GET"])
@login_required
def api_timesheets_month_get():
    from app.community_edition import abort_if_module_locked
    from app.timesheet_month_store import month_state
    from app.timesheet_signed import normalize_month
    ...
    try:
        ...
        return jsonify({"error": str(exc)}), 400
    state = month_state(current_user.id, month)
    ...
    out = {
        ...
        "draft_saved_at": state.get("draft_saved_at") or "",
        "export": {
            "saved": bool(export),
            "filename": (export or {}).get("filename") or "",
            "saved_at": (export or {}).get("saved_at") or "",
            "download_url": url_for("intranet.api_timesheets_export_download", month=month) if export else "",
        },
        "signed": {
            "uploaded": bool(signed),
            "original_name": (signed or {}).get("original_name") or "",
            "uploaded_at": (signed or {}).get("uploaded_at") or "",
            "download_url": url_for("intranet.api_timesheets_signed_download", month=month) if signed else "",
        },
    }
    return jsonify(out)


@bp.route("/api/timesheets/month", methods=["PUT"])
@login_required
def api_timesheets_month_put():
    from app.community_edition import abort_if_module_locked
    from app.timesheet_month_store import save_month_draft
    ...
    if not isinstance(payload, dict):
        return jsonify({"error": "invalid payload"}), 400
    try:
        out = save_month_draft(current_user.id, payload)
    except ValueError as exc:
        return jsonify({"error": str(exc)}), 400
    except Exception as exc:
        return jsonify({"error": str(exc) or "Could not save timesheet."}), 400
    return jsonify({"ok": True, "month": out.get("month"), "saved_at": out.get("saved_at")})


@bp.route("/api/timesheets/signature", methods=["GET"])
@login_required
def api_timesheets_signature_get():
    from app.community_edition import abort_if_module_locked
    from app.timesheet_user_signature import get_user_signature

    abort_if_module_locked("timesheets")
    out = get_user_signature(current_user.id)
    return jsonify({"ok": True, "image": out.get("image") or ""})


@bp.route("/api/timesheets/signature", methods=["PUT"])
@login_required
def api_timesheets_signature_put():
    from app.community_edition import abort_if_module_locked
    from app.timesheet_user_signature import save_user_signature
    ...
    try:
        out = save_user_signature(current_user.id, image)
    except ValueError as exc:
        return jsonify({"error": str(exc)}), 400
    return jsonify({"ok": True, "image": out.get("image") or ""})


@bp.route("/api/timesheets/export/download", methods=["GET"])
@login_required
def api_timesheets_export_download():
    from app.community_edition import abort_if_module_locked
    from app.timesheet_month_store import export_file_path, get_month_export
    from app.timesheet_signed import normalize_month
    ...
    try:
        ...
        return jsonify({"error": str(exc)}), 400
    path = export_file_path(current_user.id, month)
    if not path:
        return jsonify({"error": "not found"}), 404
    info = get_month_export(current_user.id, month) or {}
    name = str(info.get("filename") or path.name)
    return send_file(path, mimetype="application/pdf", as_attachment=True, download_name=name)


@bp.route("/api/timesheets/pdf", methods=["POST"])
@login_required
def api_timesheets_pdf():
    from io import BytesIO

    from app.community_edition import abort_if_module_locked
    from app.timesheet_month_store import save_month_draft, save_month_export
    from app.timesheet_pdf import build_timesheet_pdf

    abort_if_module_locked("timesheets")
    payload = request.get_json(force=True, silent=True) or {}
    if not isinstance(payload, dict):
        return jsonify({"error": "invalid payload"}), 400
    from app.timesheet_settings import timesheet_settings_for_pdf

    branding = timesheet_settings_for_pdf()
    try:
        pdf_bytes = build_timesheet_pdf(
            payload,
            company_lines=branding.get("company_lines") or [],
            logo_path=branding.get("logo_path"),
        )
    except Exception as exc:
        return jsonify({"error": str(exc) or "Could not build PDF."}), 400

    try:
        save_month_draft(current_user.id, payload)
        period_start = str(payload.get("period_start") or "")[:10]
        month_label = str(payload.get("month") or period_start[:7] or "")[:7]
    except ValueError as exc:
        return jsonify({"error": str(exc)}), 400

    consultant = str(payload.get("consultant_name") or current_user.full_name or current_user.username or "timesheet").strip()
    try:
        y, m = month_label.split("-")
        month_name = datetime(int(y), int(m), 1).strftime("%B %Y")
    except Exception:
        month_name = month_label
    safe_name = re.sub(r"[^\w\s-]", "", consultant).strip().replace(" ", " ") or "Timesheet"
    fname = f"{safe_name} Timesheet - {month_name}.pdf"
    try:
        save_month_export(current_user.id, month_label, pdf_bytes, fname)
    except ValueError as exc:
        return jsonify({"error": str(exc)}), 400
...
@bp.route("/api/timesheets/signed", methods=["GET"])
@login_required
def api_timesheets_signed_get():
    from app.community_edition import abort_if_module_locked
    ...
    try:
        ...
        return jsonify({"error": str(exc)}), 400
    info = get_signed_timesheet(current_user.id, month)
    if not info:
        return jsonify({"ok": True, "uploaded": False, "month": month[:7]})
    return jsonify(
        {
            "ok": True,
            "uploaded": True,
            ...
            "download_url": url_for("intranet.api_timesheets_signed_download", month=info["month"]),
        }
    )


@bp.route("/api/timesheets/signed", methods=["POST"])
@login_required
def api_timesheets_signed_upload():
    from app.community_edition import abort_if_module_locked
    from app.timesheet_signed import normalize_month, save_signed_timesheet
    ...
    try:
        info = save_signed_timesheet(current_user.id, month, f, f.filename)
    except ValueError as exc:
        return jsonify({"error": str(exc)}), 400
    except Exception as exc:
        return jsonify({"error": str(exc) or "Could not save signed timesheet."}), 400

    draft_raw = request.form.get("draft") or ""
    if draft_raw:
        try:
            draft_payload = json.loads(draft_raw)
            if isinstance(draft_payload, dict):
                draft_payload.setdefault("month", month)
                from app.timesheet_month_store import save_month_draft

                save_month_draft(current_user.id, draft_payload)
        except Exception:
            pass
...
@bp.route("/api/timesheets/signed/download", methods=["GET"])
@login_required
def api_timesheets_signed_download():
    from app.community_edition import abort_if_module_locked
    from app.timesheet_signed import get_signed_timesheet, normalize_month, signed_file_path
    ...
    try:
        ...
        return jsonify({"error": str(exc)}), 400
    path = signed_file_path(current_user.id, month)
    if not path:
        return jsonify({"error": "not found"}), 404
    info = get_signed_timesheet(current_user.id, month) or {}
    name = str(info.get("original_name") or path.name)
    return send_file(path, mimetype="application/pdf", as_attachment=False, download_name=name)
...
@login_required
def profile_page():
    """Signed-in user's profile (self-service edit or read-only + admin link)."""
    u = db.session.get(User, int(current_user.id))
    if not u or not u.is_active:
        abort(404)
    can_edit_self = rbac.user_uses_intranet_profile_modal(current_user)
    admin_edit_url = ""
    if not can_edit_self and rbac.user_can_access_users_admin(current_user):
        admin_edit_url = url_for("intranet.admin_page", edit_user_id=u.id)
    workforce_entry = _directory_entry_for_user(u) if _workforce_can_read() else None
    return render_template(
        "intranet_profile.html",
        nav=_nav("home"),
        profile_user=u,
        can_edit_self=can_edit_self,
        admin_edit_url=admin_edit_url,
        workforce_entry=workforce_entry,
        url_api_me_profile=url_for("intranet.api_me_profile") if can_edit_self else "",
    )
...
def user_page(user_id: int):
    """Workforce member profile (read-only; edit via Workforce overlay)."""
    if not _workforce_can_read():
        abort(403)
    u = db.session.get(User, user_id)
    if not u or not u.is_active:
        abort(404)
    entry = _directory_entry_for_user(u)
    is_self = int(current_user.id) == int(u.id)
    return render_template(
        "intranet_user.html",
        nav=_nav("directory"),
        user_entry=entry,
        is_self=is_self,
        can_edit_workforce=_workforce_can_create(),
        directory_url=url_for("intranet.directory_page"),
        profile_url=url_for("intranet.profile_page") if is_self else "",
        workforce_edit_url=(
            url_for("intranet.directory_page", user_id=u.id, workforce_edit=1) if _workforce_can_create() else ""
        ),
    )
...
def _register_enterprise_intranet() -> None:
    try:
