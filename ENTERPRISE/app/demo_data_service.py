...
_POOL_CRM_COMPANIES: list[dict[str, str]] = [
    {"name": "Bright Solutions", "website": "https://example.com/bright", "location": "Sydney"},
    {"name": "NextGen Systems", "website": "https://example.com/nextgen", "location": "Melbourne"},
    {"name": "Alpha Enterprises", "website": "https://example.com/alpha", "location": "Brisbane"},
]

_POOL_CRM_LEADS: list[dict[str, Any]] = [
    {"full_name": "Rachel Green", "email": "rachel@techflow.example.com", "company_idx": 0, "status": "New", "source": "Website"},
    {"full_name": "David Lee", "email": "david@nextgen.example.com", "company_idx": 1, "status": "Qualified", "source": "Referral"},
    {"full_name": "Sarah Johnson", "email": "sarah@alpha.example.com", "company_idx": 2, "status": "Proposal", "source": "Event"},
    {"full_name": "Mike Thompson", "email": "mike@bright.example.com", "company_idx": 0, "status": "Negotiation", "source": "Partner"},
    {"full_name": "Emily Davis", "email": "emily@nextgen.example.com", "company_idx": 1, "status": "Won", "source": "Inbound"},
]

_POOL_RESOURCE_POOL: list[dict[str, Any]] = [
    {"full_name": "Alex Rivera", "email": "demo.alex.rivera@resource.example.com", "job_title": "Security Consultant", "skills": ["ISO 27001", "IRAP"]},
    {"full_name": "Jordan Blake", "email": "demo.jordan.blake@resource.example.com", "job_title": "Data Engineer", "skills": ["Python", "AWS"]},
    {"full_name": "Casey Morgan", "email": "demo.casey.morgan@resource.example.com", "job_title": "Project Manager", "skills": ["Agile", "PRINCE2"]},
    {"full_name": "Taylor Reed", "email": "demo.taylor.reed@resource.example.com", "job_title": "UX Designer", "skills": ["Figma", "Research"]},
    {"full_name": "Sam Parker", "email": "demo.sam.parker@resource.example.com", "job_title": "Network Engineer", "skills": ["Cisco", "Fortinet"]},
]

_POOL_CLEARANCE: list[dict[str, str]] = [
    {"csid": "DEMO-001", "given": "Sarah", "family": "Johnson", "level": "Baseline", "email": "demo.sarah.johnson@example.com"},
    {"csid": "DEMO-002", "given": "Mike", "family": "Thompson", "level": "NV1", "email": "demo.mike.thompson@example.com"},
    {"csid": "DEMO-003", "given": "Emily", "family": "Davis", "level": "NV2", "email": "demo.emily.davis@example.com"},
    {"csid": "DEMO-004", "given": "James", "family": "Wilson", "level": "PV", "email": "demo.james.wilson@example.com"},
    {"csid": "DEMO-005", "given": "Ravi", "family": "Kumar", "level": "Baseline", "email": "demo.ravi.kumar@contractor.example.com"},
]
...
_SECTION_POOLS: dict[str, list[Any]] = {
    ...
    "crm_companies": _POOL_CRM_COMPANIES,
    "crm_leads": _POOL_CRM_LEADS,
    "resource_pool": _POOL_RESOURCE_POOL,
    "security_clearance": _POOL_CLEARANCE,
...
def _crm_company_by_idx() -> dict[int, CRMCompany]:
    out: dict[int, CRMCompany] = {}
    for idx, spec in enumerate(_POOL_CRM_COMPANIES):
        row = db.session.query(CRMCompany).filter(CRMCompany.name == spec["name"]).first()
        if not row:
            row = CRMCompany(
                name=spec["name"],
...
def _seed_crm_companies(items: list[dict], actor_id: int) -> int:
    added = 0
    for spec in items:
        if db.session.query(CRMCompany).filter(CRMCompany.name == spec["name"]).first():
            continue
        db.session.add(
            CRMCompany(
                name=spec["name"],
...
def _seed_crm_leads(items: list[dict], actor_id: int) -> int:
    companies = _crm_company_by_idx()
    added = 0
    for spec in items:
        email = spec.get("email") or ""
        if email and db.session.query(CRMLead).filter(CRMLead.email == email).first():
            continue
        company = companies.get(int(spec.get("company_idx") or 0))
        lead = CRMLead(
        ...
        db.session.add(
            CRMActivity(
                kind="note",
                title="Demo activity",
                body="Initial demo note for this lead.",
                lead_id=lead.id,
                company_id=(company.id if company else None),
                created_by_id=actor_id,
            )
...
def _seed_resource_pool(items: list[dict], actor_id: int) -> int:
    added = 0
    for spec in items:
        email = spec.get("email") or ""
        if email and db.session.query(ResourcePoolResource).filter(ResourcePoolResource.email == email).first():
            continue
...
def _seed_clearance(items: list[dict]) -> int:
    from app.enterprise.security_clearance_store import upsert_clearance_records
        ...
        payload.append(
            {
                ...
                "level": spec.get("level") or "Baseline",
                "email": spec.get("email") or "",
                "status": "Active",
                "agent_request_from": "Demo Agency",
                "created_at": int(time.time() * 1000),
            }
        )
    if not payload:
        return 0
    added, _updated, _n = upsert_clearance_records(payload)
    return added
...
def add_demo_data_batch(*, actor_id: int) -> tuple[dict[str, int], str]:
    ...
    items, cur, n = slice_pool("crm_companies")
    if n:
        c = _seed_crm_companies(items, actor_id)
        added["crm_companies"] = c
        progress["crm_companies"] = cur + n

    items, cur, n = slice_pool("crm_leads")
    if n:
        c = _seed_crm_leads(items, actor_id)
        added["crm_leads"] = c
        progress["crm_leads"] = cur + n

    items, cur, n = slice_pool("resource_pool")
    if n:
        c = _seed_resource_pool(items, actor_id)
        added["resource_pool"] = c
        progress["resource_pool"] = cur + n

    items, cur, n = slice_pool("security_clearance")
    if n:
        c = _seed_clearance(items)
        added["security_clearance"] = c
        progress["security_clearance"] = cur + n
